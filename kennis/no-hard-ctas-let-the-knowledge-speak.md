---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [positie, marketing, content, positioning, cloud-adoption]
bron: salomon-it
---

# No hard CTAs; let demonstrated expertise do the selling

His stance on marketing: drop the standard call-to-action machinery ('let's grab a coffee', etc.). It should simply be clear that you know your stuff, full stop. The audience is intelligent enough to find you on their own. Content should not feel like advertising even when it draws on your own Roadmap, training, and adoption framework; underline real, common challenges and show how you help, rather than pitching.

Verbatim: "Geen directe CTA als in, kopje koffie, etc. het moet duidelijk zijn dat we kennis van zaken hebben, that's it. Men weet ons te vinden, ons publiek is intelligent genoeg."

*Bron: Salomon-IT 'Salomon marketing 2020' (Marketing).*

## Verwante notities

- ["As a Service" Is a Go-To-Market Move, Not Necessarily a New Product](as-a-service-is-a-go-to-market-not-a-product.md)
- [Idea: give away Azure credit and run experience-based cloud workshops](azure-credit-giveaway-and-cloud-workshops-idea.md)
- [CGIT is about people and knowledge sharing, not just the brand](cgit-is-about-people-and-knowledge-sharing.md)
- [Stance: freely sharing knowledge is the best advertising a company can get](free-knowledge-sharing-is-the-best-advertising.md)
- [Stance: lead with the pitfalls instead of routing around them](lead-with-pitfalls-not-around-them.md)
- [Quality Over Quantity in Community Content](quality-over-quantity-in-community-content.md)
- [Salomon 2020 marketing: knowledge-first, strategic over technical, no hard CTAs](salomon-2020-marketing-strategy-knowledge-first.md)
- [Method: whitepaper downloads as a lead-generation engine](whitepaper-lead-generation-cloud-topics.md)
