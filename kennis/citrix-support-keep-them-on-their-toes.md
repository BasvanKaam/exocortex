---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [citrix, support, critique, premier-support]
bron: basvankaam-blog
---

# A candid critique of Citrix support response times

From a real field case (Oct 2014), Bas is openly critical of Citrix support: opening a case "took days, weeks even" to get somewhere, sometimes days for any reply, with an apparent time-zone gap and no follow-the-sun, despite Premier Support 24x7x365. His practical advice: enable CDF tracing, PortICA and VDA logging on all DDCs, StoreFront servers and VDIs before opening a case so you can upload logs immediately, which he reckons saves about a week. "They should make this mandatory."

He closes diplomatically: "I'm not here to bring them down or anything, I just like to keep them on their toes :-)" A characteristic move: honest, specific criticism delivered with warmth and without burning the relationship.

*Bron: blogpost 'Troubleshooting Citrix XenDesktop & XenApp 7.x ? Read this !' (2014-10-20), basvankaam.com.*

## Verwante notities

- [The Citrix XenDesktop/XenApp 7.x troubleshooting toolset](citrix-7x-troubleshooting-toolset.md)
- [Citrix support model shift to Customer Success Services](citrix-customer-success-services-2017.md)
- [Mandatory paid support is not a bad thing per se](customer-success-services-take.md)
- [Troubleshooting tip #1: document what you've done](document-your-troubleshooting.md)
- [You don't need Citrix Support all the time - learn to troubleshoot the FMA yourself](dont-need-citrix-support-all-the-time.md)
- [Support is a Differentiator, Not an Afterthought](support-as-a-differentiator-nerdio.md)
- [Run a CDF trace before you contact Citrix Support](trace-before-you-call-citrix-support.md)
