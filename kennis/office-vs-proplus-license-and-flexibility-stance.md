---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-11
tags: [office-365, positie, licensing, opex-capex, modern-workplace]
bron: salomon-whitepaper
---

# Prefer ProPlus for its licensing model and flexibility

Both Office scenarios have pros and cons, but Bas's preference in most cases goes to the Office 365 ProPlus variant. The licensing model (Opex subscription rather than a Capex one-off) plays a big role, as does the flexibility the software brings and the continuous stream of new features, which many businesses see as a major plus. He frames the choice as one that demands a solid plan or roadmap looking 5 to 10 years ahead, grounded in the right information rather than picked ad hoc.

*Bron: Salomon-IT 'Whitepaper - Ofifce 365 ProPlus implementeren en onderhouden - gecorrigeerd - Copy'.*

## Verwante notities

- [ProPlus on RDS and shared computer activation](office-365-proplus-on-rds-shared-computer-activation.md)
- [Office 365 ProPlus versus traditional Office](office-365-proplus-vs-traditional-office.md)
- [Office 365 subscription families and plans](office-365-subscription-families-and-plans.md)
- [An Office rollout is part of a wider workplace transformation](office-rollout-is-bigger-than-installing-software.md)
- [ProPlus is usually the better choice over traditional Office](position-proplus-preferred-over-traditional-office.md)
- [Why ProPlus/365 is usually preferred over perpetual Office](why-organizations-prefer-365-proplus-over-perpetual-office.md)
