---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-10
tags: [certification, azure, microsoft365, aws]
layer: reference
gedateerd: ja
bron: basvankaam-blog
---

# Three foundational cloud certs (2019)

How it stood in 2019: three entry-level cloud certifications with free official training material (only the exam itself costs money).

- Microsoft Azure Fundamentals: exam AZ-900
- Microsoft 365 Certified Fundamentals: exam MS-900
- AWS Foundational, Cloud Practitioner: exam CLF-C01

Bas pointed readers to the free Microsoft Learn paths plus Vlad Catrinescu's extended study guides for the Microsoft exams.

*Bron: blogpost 'Cloud Foundation - Boot Camp. Three (almost) free Cloud certifications' (2019-04-25), basvankaam.com.*

## Verwante notities

- [Certs are worth it, and the best study material isn't always Citrix's](certification-and-free-study-recommendations.md)
- [Citrix 2013 solutions-focused certification structure](citrix-2013-certification-program-structure.md)
- [How Citrix's solution-based certification tracks were structured (2014)](citrix-solution-based-certs-2014.md)
- [The IT (Cloud) Professional 2.0: from geek to strategic generalist](it-cloud-professional-2-0.md)
- [Podcasts as a learning channel for Cloud and EUC](podcasts-for-cloud-euc.md)
- [Re-doing the full cert path from the bottom isn't a bad idea, even for seniors](start-from-the-bottom-even-if-senior.md)
