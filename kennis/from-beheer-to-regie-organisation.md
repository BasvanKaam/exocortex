---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [regie, demand-supply, operating-model, it-organisation]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# From Beheer to Regie: The Regieorganisatie Model

Bas frames a key cloud-driven shift: the traditional management (beheer) role evolves into a directing/orchestration role (regie), shifting focus from the internal organisation to a balance between customer and supplier. This requires a new operating model, the regieorganisatie, which aligns customer demand with supplier supply. It has two core parts:

- **Demand management:** organises the demand side, focused on the customer / internal organisation. People in this role have strong affinity with business processes and how information provision supports them. Called the 'requirements perspective'.
- **Supply management:** organises the supply side, focused on the supplier. These roles handle procurement of IT services and issuing assignments. Called the 'solutions perspective', centred on expertise in market developments and IT solutions.

The IT regieorganisatie acts as a bridge between demand and supply, requiring knowledge of both business processes and the IT market. Where the traditional beheerorganisatie focused on internal processes and systems, in a regieorganisatie the employee becomes a link between the business (customer) and external suppliers, demanding new competencies: stakeholder management, communication skills, and insight into both business and IT.

## Verwante notities

- [CCoE left brain / right brain split across the Cloud Service Lifecycle](ccoe-left-brain-right-brain.md)
- [Cloud Cultural Shifts: DevOps, Automation, New Roles](cloud-cultural-shifts-overview.md)
- [Bas: the cloud doesn't make people redundant, it frees them for better work](cloud-frees-people-for-better-work.md)
- [Cloud Is an Operating Model, Not a Location](cloud-is-an-operating-model-not-a-location.md)
- [The Evolution of the CIO and CFO Role](evolution-of-cio-cfo-role.md)
- [IT as Internal Broker and the Regie-Organisatie](it-as-internal-broker-regie-organisatie.md)
- [The IT (Cloud) Professional 2.0: from geek to strategic generalist](it-cloud-professional-2-0.md)
- [ITIL Service Management in Cloud Environments](itil-in-cloud-environments.md)
- [ITIL Processes That Need Attention When Introducing Cloud](itil-process-changes-for-cloud.md)
- [Masterclass: Demand-Supply regie model for the orchestration organization](masterclass-regie-organization-demand-supply-model.md)
- [Masterclass: toolkit for working out regie processes](masterclass-regie-transition-toolkit.md)
- [Opdracht 3: Beheer-to-Regie Self-Assessment](opdracht-3-beheer-naar-regie-self-assessment.md)
- [Opdracht 3: Result Interpretation Bands](opdracht-3-result-interpretation.md)
- [Opdracht 6: The Mirror of Enterprise Architecture (EA Self-Assessment)](opdracht-6-mirror-of-enterprise-architecture.md)
- [Organizations under-invest in their own demand and supply management](position-organizations-neglect-internal-demand-supply-management.md)
- [Transformation: From Product to Service, Ownership to Usage](product-to-service-ownership-to-usage.md)
- [The shift to a regie organization is a people problem, not a technical exercise](regie-transition-is-a-people-problem-not-a-tech-exercise.md)
- [The Beheer-to-Regie Transition Is About People](regie-transition-is-about-people.md)
- [SCAF draft: the Business-IT collaboration interface ('where the magic happens')](scaf-business-it-collaboration-interface.md)
- [SCAF Process-to-Function Decomposition](scaf-process-to-function-decomposition.md)
- [The Gap Between IT and 'The Business'](the-gap-between-it-and-the-business.md)
- [The Triple-Whammy of Service Delivery Tooling Integration](triple-whammy-service-delivery-integration.md)
