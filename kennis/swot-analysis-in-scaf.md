---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [swot, scaf, strategie, analysis]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# SWOT Analysis in SCAF

A SWOT analysis is a strategic-planning method that maps strengths, weaknesses, opportunities and threats — holding up a mirror to the organisation. Both individuals and companies use it to understand their current situation, future possibilities and shortcomings; Bas notes a good example is gaining insight into the current knowledge level (revisited in chapter 8). You usually begin a SWOT with a desired end goal or specific objectives in mind, which fits SCAF's approach.

Bas's take: although a SWOT looks simple, it often yields deeper insights than expected, such as hidden interests and priorities. It is mainly a practical way to quickly get an overview of the key factors that can influence the strategy. True to his 'keep it simple' philosophy, he created his own Word-document SWOT template.

## Verwante notities

- [Opdracht 4 - Cloudkompas (XYZ-Care Health Group)](opdracht-cloudkompas-xyz-care.md)
- [SCAF: The Step-Cloud Adoptie Framework Overview](scaf-step-cloud-adoption-framework-overview.md)
- [SCAF Strategie: The Foundation Questions](scaf-strategie-fundament-questions.md)
- [SCAF Strategie Pillar: Summary of Steps](scaf-strategie-pillar-step-summary.md)
- [SCAF Strategie: Stakeholders, Actions and Results](scaf-strategie-stakeholders-and-actions.md)
- [Strategy Map (Kaplan-Norton) within SCAF Strategy phase](scaf-strategy-map-kaplan-norton.md)
- [SCAF strategy-phase templates: Summary Palet, Strategy Map, SWOT](scaf-strategy-phase-templates.md)
- [SCAF Workshop Startpunt Questions](scaf-workshop-startpunt-questions.md)
- [Strategy Map (Kaplan & Norton) in SCAF](strategy-map-kaplan-norton.md)
- [Ten characteristics and benefits of a good IT roadmap](ten-characteristics-of-a-good-it-roadmap.md)
- [What an IT strategy actually contains](what-an-it-strategy-actually-contains.md)
