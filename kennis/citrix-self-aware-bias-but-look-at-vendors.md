---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [citrix, byod, vendor-selection, opinion, consulting]
bron: basvankaam-blog
---

# A self-declared Citrix hugger who still tells you to shop around

In this Aug 2013 essay Bas openly names his bias ('Since I'm a Citrix hugger', 'I'll start with my personal favourite') and argues Citrix has the edge on mobility through the Worx/MDX ecosystem and the third-party Citrix Ready Worx program ('For me this is where Citrix has the edge over their competitors'). But, and this is the telling part, his actual *advice* is vendor-neutral and consultant-honest: 'Don't put all your eggs in one basket... Look at different vendors! Just because company X recommends product Y doesn't mean it's right for you.' He repeatedly concedes competitors offer similar functionality and that 'it all depends', leaning on the consultant's mantra. The position captures a core Bas trait: passionate advocate for his favourite tech, but intellectually honest enough to separate his enthusiasm from buying advice.

*Bron: blogpost 'BYOD... Beyond the hype' (2013-08-08), basvankaam.com.*

## Verwante notities

- [Be critical but kind; not OK to share everything you think](be-critical-but-kind.md)
- [BYOD has graduated from hype to something you can't ignore](byod-from-hype-to-unavoidable.md)
- [2013 mobile management vendor landscape](byod-vendor-landscape-2013.md)
- [Framing: reflecting on Citrix's history amid 2017 sale rumours](citrix-sale-rumours-2017-reflection.md)
- [Citrix Worx / MDX app containerization model](citrix-worx-mdx-container-model.md)
- [CTP-hosted sessions are the technical, unbiased ones](ctp-sessions-are-technical-and-unbiased.md)
- [Don't be fooled by DaaS marketing](dont-be-fooled-by-daas-marketing-position.md)
- [Firm believer in choice: spread options, but cap the platform count](firm-believer-in-choice-no-one-size-fits-all.md)
- [Platinum grows in worth, but unbundle the editions](platinum-worth-it-and-unbundle-editions.md)
- [Positie: Byte Squared is a nice add-on but not a big step](positie-bytesquared-nice-but-not-a-big-step.md)
