---
type: kennis
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [wvd, azure, skills, powershell]
layer: reference
gedateerd: ja
bron: basvankaam-blog
---

# WVD is not a click-and-go product

Even with Microsoft's good documentation and step-by-step guidance, WVD (2019 preview) was not a click-and-go product. To get it running you needed working understanding of Azure networking, comfort with PowerShell, identity/Active Directory, and image management. This is where technology partners came in: at first WVD lacked certain enterprise features, both in preview and after global availability, and partners filled that gap.

*Bron: blogpost 'When trying out the Windows Virtual Desktop (today), keep the following in mind' (2019-04-10), basvankaam.com.*

## Verwante notities

- [Bet: WVD third-party vendors are where the action is](betting-on-wvd-third-party-tooling-2020.md)
- [WVD prerequisites Nerdio still relies on: AD sync and FSLogix profiles](nerdio-wvd-prerequisites-ad-fslogix.md)
- [Microsoft deliberately leaves WVD management gaps to partners](positie-microsoft-leaves-wvd-management-to-partners-on-purpose.md)
- [Framing WVD rollout as phases, mapping partners onto each step](wvd-adoption-as-phases-with-partners.md)
- [WVD hit general availability on September 30, 2019](wvd-ga-date-sept-2019.md)
- [WVD will only get easier and cheaper, lowering the barrier](wvd-lowering-the-barrier-prediction.md)
- [WVD technology partner ecosystem reached 13 partners (2019)](wvd-partner-ecosystem-2019.md)
- [WVD preview constraints (2019)](wvd-preview-good-to-knows.md)
- [WVD tenant setup gotchas (2019 workshop)](wvd-tenant-setup-gotchas-2019.md)
- [WVD will evolve quickly because it's cloud-native](wvd-will-evolve-quickly-positie.md)
