---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [wvd, microsoft, partners, management, opinion]
bron: cgit-blog
---

# Microsoft deliberately leaves WVD management gaps to partners

Bas argues that WVD's awkward, roundabout way of configuring and maintaining environments is, in 2019, strongly dependent on partners to simplify, and that this is a conscious choice by Microsoft. His reasoning: he cannot imagine that building a solid management portal around WVD is a real challenge for Microsoft. He notes Microsoft is working on it (they showed early work at Ignite) but is unimpressed so far, 'a step in the right direction, that's it', and that they are taking their time for what are presumably valid reasons. The implied stance: there is room and reason for partners like CloudJumper to add management value while Microsoft holds back.

*Bron: CGIT blog 'CloudJumper, onbekend maakt onbemind - deel 2'.*

*Bron: Salomon-IT 'CloudJumper, onbekend maakt onbemind - deel 2' (CGIT Blogs).*

## Verwante notities

- [CloudJumper optimization features, platform VM, and pricing (2019)](cloudjumper-optimization-features-and-pricing-2019.md)
- [CloudJumper portfolio: CWMS, CW for Azure, and WVD integration](cloudjumper-portfolio-cwms-cw-for-azure-wvd.md)
- [Microsoft builds the core, Nerdio closes the management feature gap](microsoft-builds-the-plane-nerdio-fills-the-gap.md)
- [Windows Virtual Desktop 2019 launch reference](wvd-2019-launch-reference.md)
- [Framing WVD rollout as phases, mapping partners onto each step](wvd-adoption-as-phases-with-partners.md)
- [WVD is not a click-and-go product](wvd-not-click-and-go.md)
- [WVD technology partner ecosystem reached 13 partners (2019)](wvd-partner-ecosystem-2019.md)
- [WVD versus server RDS: positioning and primary use-cases](wvd-vs-server-rds-positioning-and-use-cases.md)
