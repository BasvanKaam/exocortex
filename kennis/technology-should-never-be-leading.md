---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-11
tags: [position, tooling, purpose-first, remote-work]
bron: salomon-it-blog
---

# Technology Should Never Be Leading

Bas's recurring principle, applied here to collaboration tools: choose the right tool based on the goal and what matters for your organization, and not the other way around. 'Techniek zou nooit leidend moeten zijn.' He extends this across the whole work-from-home piece by repeatedly framing technology as 'bijzaak' (a side matter) and putting routine, communication and well-being first. The position is consistent with his cloud-strategy writing where the business, not the technology, leads.

Source: Bas van Kaam, Salomon IT blog, 2020-03-18.

*Bron: blogpost 'Werken vanuit huis' (2020-03-18), salomon-it.nl.*

## Verwante notities

- [Don't think in brands or product boxes](avoid-vendor-and-product-thinking.md)
- [Put the Business Before the Technology](business-before-technology.md)
- [Choose Collaboration Tools by Purpose, Not Default](choose-collaboration-tool-by-purpose.md)
- [There is no such thing as a cloud strategy](cloud-is-not-a-strategy.md)
- [Don't stress so much about the cloud](dont-stress-about-the-cloud.md)
- [Function before technology: filter requirements against wishes](function-before-technology-requirements-method.md)
- [IoT is a mindset shift; answer the business questions before the tech](iot-strategy-before-technology.md)
- [Requirements Before Solution: Build the Business Case First](requirements-before-solution-business-case.md)
- [Technology choices and organizational change are inseparable](technology-and-organization-change-are-inseparable.md)
- [Working From Home: Routine and Productivity Practices](working-from-home-routine-and-productivity.md)
