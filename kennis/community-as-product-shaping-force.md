---
type: positie
merk: bvk
domein: nerdio
status: actief
datum: 2026-06-10
tags: [nerdio, community, philosophy]
bron: basvankaam-blog
---

# Community feedback as the force that shapes the product

Launching the Nerdio Tech Insider program (Sept 2020), Bas frames community not as marketing but as the core input to product direction: "it is one of the main reasons why our product, Nerdio Manager for WVD is shaped the way it is."

His stated deal: this is about technology alone, no marketing/sales/leads, members owe "close to nothing" in return beyond honest feedback now and then. The program is explicitly built to learn FROM the community ('This program will be about YOU, and what we can learn from YOU').

This reflects a recurring belief of his: that he gets value by being deep inside the EUC/WVD community, and that product and credibility flow from that proximity. Consistent with his earlier years running community blogs and podcasts.

*Bron: blogpost 'Introducing: Nerdio Tech Insider Community Program' (2020-09-06), basvankaam.com.*

## Verwante notities

- [Byte-Sized: Cloud Design Principles and Architectural Recommendations book](byte-sized-cloud-design-book.md)
- [Stance: take a vendor job but keep it community-first](community-first-vendor-role.md)
- [An idea born small can grow huge fast](community-small-ideas-grow-big.md)
- [Why community surveys are a simple, effective way to gather industry insight](community-surveys-as-method.md)
- [Framing: the product's job is hiding complexity](hiding-complexity-is-the-product.md)
- [Structured Idea / Feature-Request Form as a Product Feedback Loop](idea-form-feedback-loop.md)
- [Be a Learn-It-All Organization, Not a Know-It-All One](learn-it-all-not-know-it-all.md)
- [Nutanix's community-first posture impressed him](nutanix-community-first-impression.md)
- [Community involvement as the engine of his career](positie-community-as-career-engine.md)
- [Community feedback should shape the product](positie-community-shapes-the-product.md)
- [RDA is an industry first that out-shipped Citrix itself](positie-rda-industry-first.md)
- [Frame security guidance as a living starting point, not a final word](security-checklist-as-living-starting-point.md)
- [Ship sensible defaults, because people don't read the manual](ship-defaults-because-people-dont-read-manuals.md)
- [The community's success depends on us; give before you take](stance-give-to-the-community.md)
- [A senior peer group is the best sounding board](value-of-peer-sounding-board-group.md)
