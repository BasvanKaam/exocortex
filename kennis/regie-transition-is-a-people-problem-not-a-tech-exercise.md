---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [regie, governance, change-management, competencies, positie]
bron: salomon-masterclass
---

# The shift to a regie organization is a people problem, not a technical exercise

Bas frames the move from beheer (management) to regie (orchestration) explicitly as not just a technical exercise. The biggest area of attention is the competency domains, and these carry personnel consequences. His stance: organizations underestimate the human and skills side of becoming an orchestration organization, and that is where the transition actually succeeds or fails - not in the tooling.

In the masterclass he reinforces this by opening the module with a question put to colleagues/peers (a discussion exercise) rather than a tool walkthrough, deliberately surfacing the people dimension first.

Related: [[masterclass-regie-organization-demand-supply-model]]

*Bron: Salomon-IT 'Van beheer naar regie - vraag aan collega's'.*

## Verwante notities

- [A company built on family, not a family business](company-built-on-family-not-a-family-business.md)
- [From Beheer to Regie: The Regieorganisatie Model](from-beheer-to-regie-organisation.md)
- [Keep IT in control, then turn shadow IT into an enabler](keep-it-in-control-shadow-it-as-enabler.md)
- [Masterclass: Demand-Supply regie model for the orchestration organization](masterclass-regie-organization-demand-supply-model.md)
- [Masterclass: toolkit for working out regie processes](masterclass-regie-transition-toolkit.md)
- [Opdracht 3: Beheer-to-Regie Self-Assessment](opdracht-3-beheer-naar-regie-self-assessment.md)
- [Organizations under-invest in their own demand and supply management](position-organizations-neglect-internal-demand-supply-management.md)
- [The Beheer-to-Regie Transition Is About People](regie-transition-is-about-people.md)
- [What Makes SCAF Different: Vendor-Neutral and Hand-in-Hand](scaf-differentiator-vendor-neutral-hand-in-hand.md)
