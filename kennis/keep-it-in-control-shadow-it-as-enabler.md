---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [positie, shadow-it, governance, business-it-gap, masterclass]
bron: salomon-masterclass
---

# Keep IT in control, then turn shadow IT into an enabler

Bas's governance stance taught in the Masterclass: cloud makes it easy for anyone with a credit card to spin things up, and that ease is also the danger, shadow IT and over-permissioned users can get out of hand fast, so IT must keep control. But rather than only suppressing shadow IT, he reframes it as an enabler for closing the gap between IT and the business: use it as a small, simple entry point, run a workshop, apply Enterprise Architecture principles, stand up a technology/innovation board, and build the business case together. Underneath sits his belief that the shift to a broker/regie organization is mainly about people and competencies, not a technical exercise, and that IT is now "the business" rather than merely supporting it.

*Bron: Salomon-IT 'MC - Aantekeningen tbv Salomon Cloud Masterclass'.*

## Verwante notities

- [Don't Become the Department of No](avoid-the-department-of-no.md)
- [Business-IT Alignment Is A People Problem, Not A Tooling Problem](business-alignment-is-a-people-problem.md)
- [Cloud Masterclass: learning objectives](cloud-masterclass-learning-objectives.md)
- [How an Innovation Board operates, end to end](innovation-board-in-detail.md)
- [IT as Internal Broker and the Regie-Organisatie](it-as-internal-broker-regie-organisatie.md)
- [Lack of overview, knowledge, governance and Shadow IT](lack-of-overview-knowledge-governance-shadow-it.md)
- [Mind The Gap: Shadow IT as Enabler and the Entry Point](mind-the-gap-shadow-it-as-enabler.md)
- [Position: it should be Business-IT (BIT), not IT alone](position-business-technology-over-it.md)
- [The shift to a regie organization is a people problem, not a technical exercise](regie-transition-is-a-people-problem-not-a-tech-exercise.md)
