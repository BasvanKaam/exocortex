---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [positie, come-get-it, knowledge-sharing, culture, community]
bron: cgit
---

# CGIT is about people and knowledge sharing, not just the brand

In the internal promo plan Bas argues that CGIT should not be put solely at the centre: the goal is also getting to know colleagues better and sharing knowledge among each other ('Niet alleen CGIT staat hierbij centraal, ook het beter leren kennen van je collega's, kennis delen onder elkaar'). His stance: an internal community/initiative wins adoption by leading with people and shared learning, not by hard-selling the brand. He also values keeping sessions tight ('we willen het ook niet onnodig rekken').

*Bron: Salomon-IT 'CGIT intern promo plan' (CGIT Event).*

## Verwante notities

- [The Avensus group, Salomon IT and Come Get IT (CGIT): how they fit together](avensus-group-cgit-corporate-genealogy.md)
- [Byte-Sized Cloud: A Community-Written Book Project](byte-sized-cloud-community-book.md)
- [CGIT 'Come Get IT On Tour' as a Community Flywheel](cgit-community-flywheel-on-tour.md)
- [CGIT x LoginTechcast Collaboration Idea](cgit-logintechcast-collaboration-idea.md)
- [CGIT positioning: by IT pros, for IT pros](cgit-positioning-knowledge-community.md)
- [Internal Knowledge-Sharing Offsite Format](internal-knowledge-sharing-offsite-format.md)
- [No hard CTAs; let demonstrated expertise do the selling](no-hard-ctas-let-the-knowledge-speak.md)
- ['De persoon achter de professional': humanising colleagues as a content format](persoon-achter-de-professional-series-concept.md)
