---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [byod, mobility, trend, prediction]
bron: basvankaam-blog
---

# BYOD has graduated from hype to something you can't ignore

Writing in 2013, Bas's framing thesis is that BYOD has crossed from buzzword to operational reality: 'the whole bring your own device (BYOD) concept has evolved from being a hype to something we can no longer ignore and must take seriously before it's too late.' He grounds it in tablet-adoption growth figures and frames the real problem as a human tension, not a technical one: locked-down corporate devices keep data safe but make users miserable, while letting personal apps onto corporate devices risks leakage. His stance is that containerization/MAM is the way to resolve that tension, and that getting mobile management right early saves time and money. An early example of Bas spotting a shift in the EUC landscape and pushing readers to act ahead of it.

*Bron: blogpost 'BYOD... Beyond the hype' (2013-08-08), basvankaam.com.*

## Verwante notities

- [Most companies still get BYOD wrong - policy and management](byod-companies-still-get-it-wrong.md)
- [2013 mobile management vendor landscape](byod-vendor-landscape-2013.md)
- [Chromebooks require an online state of mind](chromebook-online-state-of-mind.md)
- [Chromebooks valuable but not enterprise-ready (2014)](chromebooks-not-enterprise-ready-yet-2014.md)
- [A self-declared Citrix hugger who still tells you to shop around](citrix-self-aware-bias-but-look-at-vendors.md)
- [Citrix Worx / MDX app containerization model](citrix-worx-mdx-container-model.md)
- [Genuinely enthusiastic about Citrix mobility, MDX as the standout](cloudgateway-fan-not-employee.md)
- [Why was Desktop Player for Mac so quiet?](desktop-player-for-mac-strangely-quiet.md)
- [Two ways to do Dual Persona: app wrapping vs on-device hypervisor](dual-persona-containerization-vs-hypervisor.md)
- [MDM enrollment binds both the device and the user (durable concept)](mdm-enrollment-device-then-user.md)
- [Mobile management acronyms: MDM, MAM, MIM, EMM](mdm-mam-mim-emm-glossary.md)
- [How Citrix made apps Worx-enabled with MDX wrapping](mdx-worx-app-wrapping.md)
- [Work Folders: on-premises file sync as a Dropbox alternative](work-folders-2012r2.md)
- [Workplace Join: registering devices without full domain join](workplace-join-2012r2.md)
