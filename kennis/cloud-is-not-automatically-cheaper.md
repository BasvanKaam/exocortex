---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [cloud, positie, cost, serverless, strategy]
bron: salomon-it-blog
---

# The Cloud Is Not Automatically Cheaper, And 'No Upfront Investment' Is Often False

Bas takes an explicit stance against two pieces of cloud marketing orthodoxy. First: the cloud is not cheaper by definition, 'het ligt er aan' (it depends). Second: the claim of 'no upfront investment' is, in his view, not always true. If you lift-and-shift your datacenter into the cloud, you get the same maintenance-heavy estate and no cost saving; if you do it right with Serverless/PaaS/IaC, you must invest time and money upfront before the long-term savings arrive. Either way the easy 'no investment' promise breaks. His positive position is consistent with his recurring theme: go for the best solution, always, even if that means a more traditional approach, and decide by mapping the pros and cons together with the client rather than chasing the cloud as a default. The deeper stance: the real obstacle to good cloud is mindset, people keep thinking in traditional IT terms and that, more than technology, slows adoption.

*Bron: Salomon-IT 'CLOUD – Als je doet wat je altijd al deed, krijg je wat je altijd al kreeg!' (Salomon Blogs).*

## Verwante notities

- [Bas: 'Wegwerp IT' and the Lift-and-Shift Caution](bas-wegwerp-it-and-lift-and-shift.md)
- [Citrix Cloud / Essentials commercial reality vs the cloud promise](citrix-cloud-essentials-commercial-reality-2017.md)
- [Cloud-First, But Not Everything to the Cloud](cloud-first-tenzij-not-everything-to-the-cloud.md)
- [Cloud Value Comes From Rethinking, Not Lift-And-Shift](cloud-lift-and-shift-vs-rethinking.md)
- [Cloud is not the answer to everything, but baseline knowledge is non-negotiable](cloud-needs-baseline-knowledge-not-hype.md)
- [The 'Cloud -> On-premises' counter-trend (draft angle)](cloud-to-on-premises-repatriation-angle.md)
- [Is the Cloud Cheaper? It Depends](is-cloud-cheaper-it-depends.md)
- [Position-led idea: 'to the cloud, unless'](naar-de-cloud-tenzij.md)
- [Cloud, and Citrix especially, is almost always more expensive than on-prem](position-citrix-cloud-is-almost-always-pricier-than-on-prem.md)
- [If your main reason to migrate is cost savings, rethink your strategy](position-cloud-not-cheaper-by-default.md)
- [Position: the original cloud promises must be put up for debate, not taken at face value](position-cloud-promises-need-nuance.md)
