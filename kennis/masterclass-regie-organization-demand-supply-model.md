---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [masterclass, regie, governance, operating-model, demand-supply]
layer: reference
gedateerd: nee
bron: salomon-masterclass
---

# Masterclass: Demand-Supply regie model for the orchestration organization

In the masterclass module "Van beheer naar regie" (from management to orchestration), Bas teaches the regie (brokerage/orchestration) function as the layer between Business and IT Delivery. The model splits into three columns: Business poses the demand ("Wat?" / what), the regiefunctie translates demand into supply ("Hoe?" / how), and IT Delivery handles realization. The orchestration organization owns the demand-to-supply translation rather than doing the delivery itself.

This is a teaching framing specific to the training; pair it with the supporting toolkit note.

Related: [[masterclass-regie-transition-toolkit]]

*Bron: Salomon-IT 'Van beheer naar regie - vraag aan collega's'.*

## Verwante notities

- [From Beheer to Regie: The Regieorganisatie Model](from-beheer-to-regie-organisation.md)
- [IT as Internal Broker and the Regie-Organisatie](it-as-internal-broker-regie-organisatie.md)
- [Masterclass: toolkit for working out regie processes](masterclass-regie-transition-toolkit.md)
- [Position: No Information, No Control, No Trust](position-control-information-trust-chain.md)
- [Organizations under-invest in their own demand and supply management](position-organizations-neglect-internal-demand-supply-management.md)
- [The shift to a regie organization is a people problem, not a technical exercise](regie-transition-is-a-people-problem-not-a-tech-exercise.md)
- [SCAF draft: the Business-IT collaboration interface ('where the magic happens')](scaf-business-it-collaboration-interface.md)
