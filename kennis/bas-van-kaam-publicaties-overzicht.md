---
type: kennis
merk: bvk
domein: persoonlijk
status: actief
datum: 2026-06-19
tags: [bio, publicaties, boeken, blog, cheat-sheets]
layer: reference
bron: eucnewsnuggets-dashboard
---

# Bas van Kaam: publications overview

Consolidated reference of his books, blogs, articles and cheat sheets across the EUC and cloud community. Headline counts on his own page: 550+ articles, 5 books, 14+ cheat sheets.

## Books
- **Cloud Mastery: A Practical Guide to Cloud Strategy, Leadership & Execution** (2026). Sole author, EN, 594 pp., ISBN 978-8288972034, 17 chapters, built on the SCAF framework. Amazon bestseller in its category.
- **Van de Basis tot Meester in de Cloud** (2025). Sole author, Dutch, eBook PDF, includes the SCAF toolkit (19 templates). Bestseller in its category. This is the Dutch sibling of Cloud Mastery.
- **Byte-Sized Cloud Design Principles and Architectural Recommendations** (2019). Co-author with Christiaan Brinkhoff, ISBN 978-1797692104, 152 submissions from 20 countries. Amazon bestseller.
- **Inside Citrix (extended digital edition)** (2017 to 2019). 28 chapters, free download, Goodreads 4.25/5.
- **Inside Citrix: The FlexCast Management Architecture** (2016). Sole author, 508 pp., ISBN 978-1530428526, around 830 copies sold. Amazon bestseller in its category.

## Blogs and long-form
- **basvankaam.com** (Jan 2013 to Apr 2023). 318 articles, frozen but archive stays live. Peak 40,000 visitors/month, tens of millions of total views. Topics: Citrix FMA internals, XenApp, XenDesktop, Azure, WVD/AVD, cloud strategy, security, IGEL, Nutanix, VMware, architecture deep-dives.
- **salomon-it.nl** (May 2018 to May 2020). 42 verified articles on cloud adoption, hybrid strategy, SCAF, TCO/ROI, application lifecycle, cloud exit strategy.
- **LinkedIn long-form** (2014 to 2026). 67 articles spanning the Qwise, Detron, Liquidware, Salomon IT and Nerdio eras, including Shadow AI and L&D themes.
- **Eight external platforms** (2012 to 2023, ~80+ articles): TechTarget / BrianMadden.com, detron.nl, DABCC.com, MyCUGC, Computable.nl, Intense School, Liquidware blog, Nerdio blog.

## Cheat sheets and microlearning
- 14+ reference sheets, Nov 2014 to 2026, combined views into the millions. Includes: AVD Security Guidelines v1.0 (35 measures, 6 categories, Apr 2023), Azure Reserved Instances, Remote video conferencing, WVD partner, Cloud Services v1.0+v2.0, IOPS, (I)IoT networks, XenDesktop 7.x internals v1.0 and v2.0 (71 pp.), App layering v1.0 to v3.0, Citrix IMA vs FMA.
- eucnewsnuggets.com (2025 to 2026): microlearning PDF platform, 6 PDFs covering AVD on Nutanix, Intune Policy Studio, Windows 365 for Agents, Windows 365 Flex, Nerdio Compass, AVD Hybrid + Nutanix.

## Podcasts (professional)
- **EUC News Nuggets** (2026 to present): AI-narrated microlearning podcast (voice: Finn Morgan), paired with the PDF cheat sheets.
- **EUCdigest** (2019 to 2023): 87 episodes across 3 seasons (S01 21, S02 13, S03 53). Co-founder and board member, main responsible for sponsor management and commercial deals, not the regular host.
- Guest appearances (2018 to 2021): THRIVEcast Ep. 052, Technado Ep. 171, Come Get IT Ep. 21, WVD Community Podcast Ep. 4, Login TechCast.

Note: his Dutch personal-finance podcast (Altijd in Bèta) and the Nooit Meer Blut programme are a separate venture, deliberately kept outside this brain.

*Bron: eucnewsnuggets.com Bas van Kaam dashboard (Personal Hall of Fame, May 2026).*

## Verwante notities

- [Bas van Kaam: career timeline (1999 to 2026)](bas-van-kaam-loopbaan-tijdlijn.md)
- [Bas van Kaam: credentials, awards and community programs](bas-van-kaam-credentials-en-awards.md)
- [Cloud Mastery - self-positioning and book intent](cloud-mastery-author-positioning.md)
- [Writing the FMA book was brutal but never regretted](self-publishing-fma-book-worth-it.md)
- [The blog began as a personal knowledge base, and sharing became the point](blog-as-personal-knowledge-base-origin.md)
