---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [citrix, vdi-in-a-box, positie, vdi]
bron: basvankaam-blog
---

# VIAB isn't for everyone, but with the right business case it just feels right (May 2013)

Bas is warm on VIAB while staying honest about scope: "Although it's not for everyone, given the proper Business Case (and there are plenty) it just feels right." He praises how easy it is to install and manage, calls the Tech Preview roll-out "a nice and smooth roll out" that took only a few hours, and is intrigued by the claimed scalability ("proven to be quite robust ... even up to thousands of desktops"). His signature decision framing closes it out: choosing between VIAB and XenDesktop "all depends" - you can be done "with a snap of the finger" or need detailed research. A representative example of his pragmatic, business-case-first stance on tooling.

*Bron: blogpost 'Project Curacao complete overview' (2013-05-14), basvankaam.com.*

## Verwante notities

- [Firm believer in choice: spread options, but cap the platform count](firm-believer-in-choice-no-one-size-fits-all.md)
- [On over-engineering HA: simplicity often wins, but it all depends](ha-it-all-depends.md)
- [VDI-in-a-Box: a single all-in-one VDI appliance (how it worked)](vdi-in-a-box-all-in-one-appliance.md)
- [VDI-in-a-Box: the simplified all-in-one VDI appliance](vdi-in-a-box-architecture.md)
- [VDI-in-a-Box served a real niche: simple, cheap, small-scale VDI](vdi-in-a-box-simple-vdi-niche.md)
- [Simplicity is a feature, not a drawback (VIAB)](viab-simplicity-is-strength-position.md)
- [VIAB vs XenDesktop: simplicity vs flexibility tradeoffs (2013)](viab-vs-xendesktop-tradeoffs.md)
