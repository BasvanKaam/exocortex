---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-11
tags: [citrix-cloud, cost, cloud-economics, position]
bron: salomon-it-blog
---

# Cloud, and Citrix especially, is almost always more expensive than on-prem

Bas's stance in his ComeGetIT review, pushing back on cost-saving claims for cloud Citrix: a direct on-premises comparison does not produce the savings people assume. 'Cloud, and Citrix especially, is almost always more expensive than a comparable on-prem solution.' The cost-saving framing collapses further once you account for the licensing lock-in (minimum 25 Essentials licenses, 1- or 3-year commitments), which removes the scale-down benefit.

This is consistent with his broader position that cloud does not automatically save money and that cost-saving is the wrong primary driver, but here it is sharpened specifically to Citrix-in-the-cloud economics versus on-prem.

Source: Bas van Kaam, internal feedback document on ComeGetIT, Salomon IT era.

*Bron: Salomon-IT 'Feedback op ComeGetIT' (Salomon Blogs).*

## Verwante notities

- [Citrix Cloud / Essentials commercial reality vs the cloud promise](citrix-cloud-essentials-commercial-reality-2017.md)
- [Moving to the cloud does not automatically save money](cloud-does-not-automatically-save-money.md)
- [The Cloud Is Not Automatically Cheaper, And 'No Upfront Investment' Is Often False](cloud-is-not-automatically-cheaper.md)
- [The 'Cloud -> On-premises' counter-trend (draft angle)](cloud-to-on-premises-repatriation-angle.md)
- [If cost saving is your primary cloud driver, reconsider your strategy](cost-saving-is-the-wrong-primary-cloud-driver.md)
- [The key line between Essentials and full Citrix Cloud: VDA placement](essentials-vs-full-citrix-cloud-vda-placement.md)
