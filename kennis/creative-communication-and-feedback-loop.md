---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [communication, engagement, feedback, culture]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# Creative Communication and Putting Responsibility on Staff

Bas's advice on making cloud transformation feel less abstract for employees. A single announcement isn't enough — communication must be regular and consistent, and not only formal via email.

Use physical spaces creatively: the hall, cafeteria, corridors, elevators, even the stairwell, to carry the message and keep constant awareness. Use visuals like infographics or even cartoons (keep it light) at strategic spots to convey the importance of the changes humorously and accessibly.

Develop a clear home-work policy: expectations around remote work, how performance is measured (Bas flags this as a hard topic), which tools to use, and how to secure data. Involve the user organisation as much as possible.

Most important: listen attentively and act on it. By involving staff and putting part of the responsibility on them, they feel engaged — with all the benefits that brings.

## Verwante notities

- [Bas's experience: feedback from early phases builds tighter teams](bas-experience-feedback-from-early-phases.md)
- [Bas on feedback as a continuous process from the insight phase](bas-feedback-continuous-process.md)
- [Bas's contrarian tip: get feedback from prospects you lost, not just customers](bas-learn-from-lost-prospects.md)
- [Bas on the helpdesk as first point of contact](bas-on-the-helpdesk-experience.md)
- [Bas's Roadmap Is Driven by Reader Feedback](bas-reader-feedback-as-roadmap.md)
- [Bas's 'van proberen kun je leren' philosophy](bas-van-proberen-kun-je-leren.md)
- [Hey... Biiieeerrrrrtje? — Early Team Bonding](biertje-team-bonding-aside.md)
- [Bas: the cloud doesn't make people redundant, it frees them for better work](cloud-frees-people-for-better-work.md)
- [Cultural Shift: Communicate, Communicate, Communicate](cultural-shift-communicate-communicate.md)
- [Bas on 'Angst': the human side of cloud adoption](fear-and-the-human-side-of-cloud-adoption.md)
- [Mind The Gap: Shadow IT as Enabler and the Entry Point](mind-the-gap-shadow-it-as-enabler.md)
- [Role-Specific Security Training (Don't Call Staff the Weakest Link)](people-are-the-weakest-link-training.md)
- [Resistance: cloud is more than technology](resistance-to-change-in-cloud-adoption.md)
- [The Technology / Innovation Board](technology-innovation-board.md)
