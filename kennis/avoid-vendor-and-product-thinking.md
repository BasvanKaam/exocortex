---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [vendor-independence, position, consulting]
bron: salomon-it-blog
---

# Don't think in brands or product boxes

Bas's stance: he does not like to think in boxes, specific brands or products. A vendor end-of-life (here Citrix XenApp 6.5) is merely an example trigger, not a reason to pick a successor product. The right question is not "which upgrade?" but "which route fits you best?" There are always multiple roads to Rome, including but not limited to the cloud.

Underlying argument: it is always wise to periodically ask whether earlier choices are still the right ones and whether you are ready for what is coming. Look further and think differently rather than defaulting to the familiar product.

*Bron: blogpost 'Applicatie lifecycle en delivery, tijd voor een heroverweging' (2018-06-04), salomon-it.nl.*

## Verwante notities

- [Full Cloud Migration Is Rarely a Good Idea](full-cloud-is-rarely-a-good-idea.md)
- [We Give the Insight, You Make the Choice](give-insight-client-decides.md)
- [OS Migration Is More Than Swapping the OS](os-migration-is-more-than-the-os.md)
- [Don't let any single vendor spook you](positie-dont-let-any-single-vendor-spook-you.md)
- [At the Bigger Picture, Stay Vendor-Agnostic](positie-vendor-agnostic-at-the-bigger-picture.md)
- [SCAF Pillars and Design Principles](scaf-pillars-and-design-principles.md)
- [Technology Should Never Be Leading](technology-should-never-be-leading.md)
- [Vendor partner congresses are marketing-first roadmap events](vendor-congresses-are-marketing-roadmaps.md)
- [Cloud Advice Must Be Vendor-Independent](vendor-independent-cloud-advice.md)
