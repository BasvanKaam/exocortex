---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [wfh, remote-work, covid]
bron: basvankaam-blog
---

# WFH is not going away anytime soon

Writing at the end of 2020 (mid-COVID), Bas frames working from home as a durable shift rather than a temporary measure: "Since WFH isn't going away anytime soon, I decided to again upgrade my WFH setup." This is the worldview behind much of his EUC content in this period: remote work as the new baseline, which is the context for his interest in workspace-aggregation tooling (e.g. the Awingu episode weeks later) and end-user experience.

*Bron: blogpost 'I'll show you mine, if you show me yours (WFH setup)' (2020-12-24), basvankaam.com.*

## Verwante notities

- [Awingu as an HTML5 workspace aggregator](awingu-html5-workspace-aggregator.md)
- [Bas van Kaam 2020 career milestones](bas-2020-career-milestones.md)
- [Bet: WVD third-party vendors are where the action is](betting-on-wvd-third-party-tooling-2020.md)
- [The browser as the aggregation layer / new OS](browser-as-the-new-os.md)
- [How the conferencing cheat sheet was researched](conferencing-cheat-sheet-method.md)
- [HTML5 browser clients: appeal and historical pain points](html5-browser-client-tradeoffs.md)
- [Switching monitor resolution from a Stream Deck via Display Changer](stream-deck-resolution-switch-display-changer.md)
- [Bas's 2020 WFH / streaming kit-list](wfh-setup-kitlist-2020.md)
- [Treating the job as a hobby you'd do anyway](work-as-hobby-philosophy.md)
