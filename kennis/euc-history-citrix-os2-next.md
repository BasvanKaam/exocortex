---
type: kennis
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [EUC, history, Citrix, community, podcast]
layer: reference
gedateerd: ja
bron: basvankaam-blog
---

# EUC history: OS/2 apps on NeXT via Citrix

From EUCdigest episode 15 with industry veteran Steve Greenberg: an early-EUC anecdote where he made OS/2 applications available on NeXT computers using Citrix technology, after starting his career as an audio engineer. The episode's throughline is that EUC has evolved a great deal yet so much has stayed the same, and that the community played an important role in the early days, eventually leading to the EUC Masters Retreat.

*Bron: blogpost 'EUCdigest Podcast EPISODE 15 – EUC Use Cases Over The Years' (2020-07-14), basvankaam.com.*

## Verwante notities

- [Citrix origins: from Citrus Systems to WinFrame, and the birth of ICA](citrix-origins-and-ica-birth.md)
- [Framing: reflecting on Citrix's history amid 2017 sale rumours](citrix-sale-rumours-2017-reflection.md)
- [Citrix/EUC community podcasts](euc-citrix-community-podcasts.md)
- [EUC-X: an umbrella for community side-projects (2019)](euc-x-community-platform.md)
- [EUC-X renamed to EUCdigest (2019)](eucdigest-rename.md)
- [EUCdigest: vendor-agnostic, 100% community-driven EUC](eucdigest-vendor-agnostic-community.md)
- [How Microsoft RDP descended from Citrix's ICA](rdp-descended-from-citrix-ica.md)
