---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [innovation, trends, opinion, war-story, customer-focus]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# Bas's reframe: chase trends and innovation only in moderation

Bas pushes back on trend-chasing: too much emphasis on following new developments distracts from the daily focus on a well-functioning cloud environment, supporting your users, and practical improvements. It is unrealistic to expect every new technology to be a decisive success within months or even years. Customers want help with their *current* situation and immediate needs — they don't jump onto every new trend. His hard-won line: "Ik heb dit te vaak fout zien gaan, helaas" (I've seen this go wrong too often, sadly). IT innovation needs serious investment, and customers want to see the value of existing solutions before taking on new risk. Stay current — but don't overdo it; focus first on what you're good at and what customers expect.

A characteristic national aside: in his view the Netherlands is far ahead of other European countries, with above-average knowledge levels even compared with the United States — "al verwacht je dat misschien niet. Mooi voor ons natuurlijk."

## Bron-citaten (NL, verbatim)

> Ik heb dit te vaak fout zien gaan, helaas.
>
> Nederland ligt ver voor als je het vergelijkt met andere Europese landen.
>
> Hoewel vernieuwing belangrijk is, kan te veel nadruk op nieuwe dingen afleiden van de dagelijkse focus op een goed functionerende Cloudomgeving, ondersteunen van gebruikers en doorvoeren van praktische verbeteringen.

## Verwante notities

- [Bas on the Cloud roadmap as a strategic tool](bas-cloud-roadmap-as-strategic-tool.md)
- [Bas's contrarian tip: get feedback from prospects you lost, not just customers](bas-learn-from-lost-prospects.md)
- [The Gartner Hype Cycle and its five stages](gartner-hype-cycle.md)
- [How an Innovation Board operates, end to end](innovation-board-in-detail.md)
- [Where to 'park' innovation: Innovation/Technology Board, Solutions Board, Office of the CTO](innovation-board-technology-board-office-of-cto.md)
- [Innovation Comes From Outside the Firewall](innovation-comes-from-outside-the-firewall.md)
- [Obvious cloud trends (multi-cloud, edge, zero trust, serverless, green)](obvious-cloud-trends-list.md)
- [Everett Rogers' technology-adoption categories mapped to the hype cycle](rogers-technology-adoption-categories.md)
