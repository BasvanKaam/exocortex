---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [citrix, workspacepod, vdi-in-a-box, opinion]
bron: basvankaam-blog
---

# WorkspacePod is not an honest VIAB replacement (yet)

Stance (Jan 2015): Bas is fine with VIAB being killed - it was 'already dead, just not buried.' What bothers him is the framing of WorkspacePod as its replacement. Citrix CEO Mark Templeton said VIAB would be replaced by 'a simpler and more price-competitive version of VDI,' and Bas's read is that WorkspacePod in its current form is the opposite of simple: 'it's far from easy and simple.' The Moonshot/Sanbolic config he judges 'pretty safe to say won't play a role in replacing a product like VIAB'; the DataOn 'Cluster-in-a-Box' comes closer.

Notably, after a Twitter debate with Citrix folks he softened the article and conceded they had a point - he agreed to wait for the final product before passing final judgement, while standing by his position that 'with all the information out there today... it isn't that hard to see that in its current form it won't be an honest replacement of VIAB.' This shows his early willingness to publicly restate an opinion when challenged, without fully backing down.

*Bron: blogpost 'Citrix kills VDI-in-a-Box and the Sanbolic acquisition - my thoughts' (2015-01-30), basvankaam.com.*

## Verwante notities

- [Citrix Summit 2015: Receiver X1, Framehawk, XenMobile 10](citrix-summit-2015-announcements.md)
- [Delivering a desktop OS multi-tenant via dedicated Moonshot cartridges](moonshot-desktop-os-multi-tenancy-licensing.md)
- [HP Moonshot as HDI: physical desktops, no hypervisor layer](moonshot-hdi-physical-no-hypervisor.md)
- [Publish the draft and let the community fact-check it](publish-the-draft-crowdsource-the-facts.md)
- [Shows his 90% certainty and his confusion rather than bluffing](shows-his-uncertainty-instead-of-bluffing.md)
- [VDI-in-a-Box: a single all-in-one VDI appliance (how it worked)](vdi-in-a-box-all-in-one-appliance.md)
- [VDI-in-a-Box served a real niche: simple, cheap, small-scale VDI](vdi-in-a-box-simple-vdi-niche.md)
- [Simplicity is a feature, not a drawback (VIAB)](viab-simplicity-is-strength-position.md)
- [VIAB vs XenDesktop: simplicity vs flexibility tradeoffs (2013)](viab-vs-xendesktop-tradeoffs.md)
- [You can judge a product without hands-on, if you do the research](you-can-have-an-opinion-without-hands-on.md)
