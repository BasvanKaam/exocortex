---
type: kennis
merk: bvk
domein: persoonlijk
status: actief
datum: 2026-06-09
tags: [study, certification, self-investment, it-professional-2.0]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# Bas on continuous study and self-investment

Bas loves studying and has earned dozens of certificates over the years — nearly two or more every year. He admits some of those subjects and certificates turned out to be of little or no use to him, yet he believes you learn something from almost everything, as long as it makes some sense.

His argument: an IT leader must understand at least something of the cloud dynamic, just as today's IT'er needs a more strategic mindset. You learn that by doing, but also by studying — self-education, investing in yourself. How you do it doesn't matter, as long as you do it. Think about the topics you'll deal with in the coming year(s), or which direction you most want to go. Thinking ahead has never harmed anyone. Like his earlier advice about ten pages a day, it works step by step, bit by bit.

## Bron-citaten (NL, verbatim)

> hoe je het doet, maakt niet uit als je het maar doet
>
> Vooruitdenken heeft nog nooit iemand kwaad gedaan.

## Verwante notities

- [Bas's Closing Reflection: Good Questions Always Move You Forward](bas-closing-reflection-good-questions.md)
- [Bas: IaC Creates Dangerous Key-Person Dependency](bas-iac-key-person-risk.md)
- [Bas's comfort-zone war story: the presentation training](bas-on-stepping-out-of-comfort-zone.md)
- [Bas's research workflow: pen-and-paper first, then digitalise](bas-pen-to-digital-research-workflow.md)
- [Bas on growth: lean into strengths, nudge weaknesses, and ask questions](bas-play-to-your-strengths.md)
- [Bas's reading habit: the 10-pages-a-morning method](bas-reading-habit-and-method.md)
- [Bas's recommended reading list](bas-recommended-booklist.md)
- [Bas van Kaam: experience, character and the people behind the book](bas-van-kaam-bio-and-character.md)
- [Bas's year as a competence manager](bas-year-as-competence-manager.md)
- [Cloud ideas for the modern, attractive employer](cloud-ideas-for-the-modern-attractive-employer.md)
- [Three (Almost) Free Foundational Cloud Certifications](free-foundational-cloud-certifications.md)
- [The IT (Cloud) Professional 2.0: from geek to strategic generalist](it-cloud-professional-2-0.md)
- [Key Takeaways Ch.15: the modern IT professional](key-takeaways-it-professional-and-cloud.md)
- [The Learning Pyramid (National Training Laboratories)](learning-pyramid.md)
- [Podcasts as a learning channel for Cloud and EUC](podcasts-for-cloud-euc.md)
- [Willing to co-invest in building his own training business](position-co-invest-in-your-own-training-business.md)
- [Re-doing the full cert path from the bottom isn't a bad idea, even for seniors](start-from-the-bottom-even-if-senior.md)
