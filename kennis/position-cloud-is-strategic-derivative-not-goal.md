---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [cloud-governance, position, strategy, ict-vision]
layer: reference
gedateerd: nee
attributie: weolcan-wcgf
bron: salomon-cloud-governance
---

# Position: Cloud Is a Strategic Derivative, Not a Goal in Itself

> Referentie uit het Weolcan Cloud Governance Framework (WCGF). Niet Bas zijn eigen IP, hier bewaard als naslag.

The framework holds that cloud is not an end in itself but an enabler for becoming a Modern Enterprise. No normal organization has "provide cloud services" as a strategic objective, so IT and cloud strategy should be derived top-down from the Digital Strategy, which itself serves business transformation.

The corollary stance: the chosen business strategy (for example Operational Excellence versus Product Leadership) dictates IT needs, so there is no one-size-fits-all cloud approach. He pushes against treating cloud adoption as a technology project, insisting it be anchored in strategic business goals.

*Bron: Salomon-IT 'ICT Vision - Cloud Governance'.*

## Verwante notities

- [Cloud Is an Operating Model, Not a Location](cloud-is-an-operating-model-not-a-location.md)
- [There is no such thing as a cloud strategy](cloud-is-not-a-strategy.md)
- [What makes a good Cloud Strategy](cloud-strategy-quality-criteria.md)
- [Functionality before technology; business objectives drive cloud adoption](functionality-before-technology-ea-principle.md)
- [ICT Vision: IT and Cloud Strategy Derive from the Digital Strategy](ict-vision-strategy-derivation-from-digital-strategy.md)
- [Mapping IT Capabilities to Strategic Business Goals](map-it-capabilities-to-strategic-business-goals.md)
- [Decide Why You're in the Cloud Before You Migrate](name-your-cloud-driver-first.md)
- [Nested Governance: Corporate over IT over Cloud](nested-governance-corporate-it-cloud.md)
- [Position: Business Objectives Must Drive Cloud Investments](position-business-objectives-drive-cloud-investments.md)
