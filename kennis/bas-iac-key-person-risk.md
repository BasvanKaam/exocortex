---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [iac, positie, key-person-risk, team, passion]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# Bas: IaC Creates Dangerous Key-Person Dependency

A second Bas practice example: an internal IT team of about five people manages a few thousand users and the associated Azure infrastructure, using IaC to keep it all running, patch, spin up new systems, auto-start/stop machines, etc. Two of the five are highly driven and skilled at IaC. Bas's warning: what happens when one of those two leaves (which does happen in this field), or when one or both is sick, on long holiday, etc.? You create a dangerous dependency if you're not careful, and it starts with being aware of the situation.

Bas also has a strong, characteristic opinion on *who* can do IaC: just pointing at someone on your team to "go learn IaC" doesn't work. People who are good at and driven by IaC are usually not your average IT'er; they spend a lot of time on self-education, love difficult situations, and are often working late evenings or weekends finding solutions, writing blogs, preparing presentations. IaC is not for everyone; you need a certain passion for it.

## Bron-citaten (NL, verbatim)

> Wat gebeurt er als een of beide personen ziek zijn, voor langere tijd op vakantie gaan en zo verder? Ook hier, als je niet oppast, creëer je een (gevaarlijke) afhankelijkheid. Daar is natuurlijk iets aan te doen, maar het begint bij het besef van de situatie.
>
> IaC is niet voor iedereen weggelegd, je moet er een bepaalde passie voor bezitten.

## Verwante notities

- [Bas's War Story: The IaC Dependency Trap](bas-iac-dependency-trap.md)
- [Bas on continuous study and self-investment](bas-on-continuous-study-and-self-investment.md)
- [Cloud-Native: Designed for the Cloud from the Start](cloud-native-definition.md)
- [Infrastructure as Code (IaC) and Its Benefits](infrastructure-as-code-basics.md)
- [The IT (Cloud) Professional 2.0: from geek to strategic generalist](it-cloud-professional-2-0.md)
- [Key-person dependency on automation is a business risk](key-person-dependency-as-business-risk.md)
- [Concentrated cloud knowledge in a few seniors is a business risk](knowledge-leaving-the-company-as-a-real-risk.md)
- [Don't put all your eggs in one basket: knowledge as a single point of failure](knowledge-single-point-of-failure.md)
- [Bas's 'one is none' rule for resilience](one-is-none-rule.md)
- [Blind faith in automation will not ensure happy outcomes](position-stay-in-control-of-your-control.md)
