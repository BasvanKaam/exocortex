---
type: positie
merk: bvk
domein: nerdio
status: actief
datum: 2026-06-11
tags: [community, product, feedback-loop, wvd]
bron: nerdio-blog
---

# Community feedback should shape the product

Bas's stance: community is not a marketing channel, it is how you build the right product. He states Nerdio has experienced the power of community firsthand on multiple occasions, and that it is 'one of the main reasons why our product, Nerdio Manager for WVD is shaped the way it is.'

The people worth listening to are the practitioners who visit and talk to hundreds of customers on a global scale: they tell you what is missing and what should be improved. That feedback is of great value and justifies building a community of your own rather than only participating in others'. The relationship has to be genuinely no-strings: members give honest feedback, the vendor does not extract leads or apply sales pressure in return.

*Bron: Nerdio-content 'Announcing - The Nerdio Tech Insider Community Program' (Blogs).*

## Verwante notities

- [Bas's Roadmap Is Driven by Reader Feedback](bas-reader-feedback-as-roadmap.md)
- [Community feedback as the force that shapes the product](community-as-product-shaping-force.md)
- [Community feedback should visibly shape the product](community-feedback-shapes-the-product.md)
- [Build a practitioner community as a go-to-market motion](community-program-as-go-to-market-idea.md)
- [Structured Idea / Feature-Request Form as a Product Feedback Loop](idea-form-feedback-loop.md)
- [Idee: Nerdio Tech Insider Community Program](idee-nerdio-tech-insider-community-program.md)
- [Idea: a sales-free tech community feedback program](idee-nerdio-tech-insider-community-program-2.md)
- [Be a Learn-It-All Organization, Not a Know-It-All One](learn-it-all-not-know-it-all.md)
- [Don't hide costs from customers: full-disclosure pricing builds trust](radical-pricing-transparency-position.md)
