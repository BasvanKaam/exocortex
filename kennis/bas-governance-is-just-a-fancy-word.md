---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [governance, opinion, reframe, ccoe, mindmap]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# Bas's reframe: governance is just a fancy word

Bas insists that, just as with setting up a CCoE, governance is not reserved only for (large) enterprise organizations even if it sounds that way. You can set up a CCoE and governance as large, small or specific as you want — there are no rules. The principle: use only what applies to your company and project, nothing more, nothing less. Years ago he created a mindmap covering governance and all the aspects that could come into play; it shows governance can span many topics and departments.

His signature deflation of the buzzword: 'governance is niets meer dan een duur woord voor alles gewoon goed in de gaten houden en aansturing geven waar en wanneer nodig' — governance is nothing more than an expensive word for simply keeping a good eye on everything and giving direction where and when needed.

## Bron-citaten (NL, verbatim)

> En vergeet niet… governance is niets meer dan een duur woord voor alles gewoon goed in de gaten houden en aansturing geven waar en wanneer nodig.

## Verwante notities

- [Bas: Pragmatism on Governance and Process Documentation](bas-governance-processes-pragmatism.md)
- [Bas on documentation: the 'undervalued child' of IT](bas-on-documentation-the-undervalued-child.md)
- [Bas on handover: 'simple is better'](bas-on-simplicity-handover-checklist.md)
- [Promoting the CCoE: the 'kindergarten' war story and strategies](bas-promoting-the-ccoe-kindergarten-war-story.md)
- [A CCoE Is Not Just for Large Enterprises](ccoe-not-just-for-enterprise.md)
- [The CCoE's role in AVG compliance and governance](ccoe-role-in-avg-compliance.md)
- [Cloud Center of Excellence (CCoE): Definition and Why Early](cloud-center-of-excellence-definition.md)
- [Governance Definition and Strategic Hierarchy](governance-definition-and-strategic-hierarchy.md)
- [Lack of overview, knowledge, governance and Shadow IT](lack-of-overview-knowledge-governance-shadow-it.md)
- [Position: No Information, No Control, No Trust](position-control-information-trust-chain.md)
- [SCAF Cloud governance: putting the dots on the i](scaf-cloud-governance-checklist.md)
- [SCAF Cloud Governance: The Nine Domains (Overview)](scaf-cloud-governance-domains-overview.md)
- [Bas: use the SCAF toolkit as inspiration, not a checklist](scaf-toolkit-use-it-as-inspiration.md)
- [What IT Governance Aims to Achieve](what-it-governance-aims-to-achieve.md)
