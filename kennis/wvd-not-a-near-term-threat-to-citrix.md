---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [wvd, citrix, daas, prediction, positie]
bron: basvankaam-blog
---

# WVD is not a near-term threat to Citrix; enterprise features still favor CTX

In this March 2019 fact list, Bas offers his read on the Citrix-vs-WVD question. His call: 'Will the WVD be a threat to Citrix? It might be in the distant future but not on short notice.' His reasoning is that Citrix has built (or is building) its own DaaS solution on top of WVD, and that most enterprise features are still lacking in WVD, 'this is where CTX still shines'. He frames WVD's roadmap as long and impressive, and is candid that the picture could 'turn into old news quickly'. He also surfaces the dissenting community voice (Claudio Rodrigues, @crod, and his 'WVD BS' talks) rather than presenting only hype. A measured prediction from the pivot moment when Microsoft entered Citrix's core territory.

*Bron: blogpost 'The Windows Virtual Desktop - what we know so far. 83 facts listed' (2019-03-19), basvankaam.com.*

## Verwante notities

- [Bet: WVD third-party vendors are where the action is](betting-on-wvd-third-party-tooling-2020.md)
- ['Finally' one bill, but the 25-user minimum rules out small shops](citrix-managed-desktop-finally-one-bill.md)
- [Citrix Managed Desktop: single-bill DaaS on Citrix-managed Azure](citrix-managed-desktop-model-2019.md)
- [Don't be fooled: DaaS marketing sells a client desktop you aren't actually getting](daas-marketing-vs-reality.md)
- [Microsoft is suddenly winning everything (2017 observation)](microsoft-winning-everything-2017.md)
- [WVD hit general availability on September 30, 2019](wvd-ga-date-sept-2019.md)
- [WVD roadmap signals from the 2019 workshop](wvd-roadmap-signals-2019.md)
- [WVD will evolve quickly because it's cloud-native](wvd-will-evolve-quickly-positie.md)
- [XenApp Express: the Citrix/Microsoft replacement for Azure RemoteApp](xenapp-express-replaces-azure-remoteapp.md)
