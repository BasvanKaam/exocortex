---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [citrix, netscaler, licensing, opinion]
bron: basvankaam-blog
---

# Credit to Citrix for simplifying NetScaler licensing without a price grab

Bas's take in July 2014: Citrix folding the Access Gateway platform license into all editions 'took a step in the right direction, simplifying the licensing structure slightly without forcing the consumer to spend more money then needed.' Notable as praise rather than critique, and as an early instance of him watching vendor licensing changes through a cost-to-customer lens. The post also documents his honesty process: he had originally published details obtained under NDA, found they had shifted, and openly revised the article with an apology rather than quietly editing.

*Bron: blogpost 'Citrix NetScaler (10.5) licensing. What's new with Access Gateway!' (2014-07-01), basvankaam.com.*

## Verwante notities

- [Citrix licensing: user, device and concurrent](citrix-licensing-models.md)
- [NetScaler 10.5 made ICA proxy built-in and unlimited](netscaler-105-ica-proxy-built-in.md)
- [NetScaler edition licenses and Pay-as-you-Grow](netscaler-adc-edition-licenses-pay-as-you-grow.md)
- [NetScaler ADC vs Gateway: same box, different license](netscaler-adc-vs-gateway-licensing.md)
- [Platinum grows in worth, but unbundle the editions](platinum-worth-it-and-unbundle-editions.md)
- [Positie: the cloud blocker is Microsoft licensing, not the product](positie-blame-microsoft-licensing-not-citrix.md)
- [The XenDesktop App Edition licensing fiasco and XenApp's comeback](xenapp-licensing-fiasco-and-comeback.md)
