---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [community, collaboration, positie]
bron: basvankaam-blog
---

# If you want to go far, go together

Bas frames the Byte-Sized success around the proverb "If You Want To Go Fast, Go Alone. If You Want To Go Far, Go Together" and the line "The force of the community is strong." The whole project is his proof-point that distributed community contribution outperforms going solo, and that community work should give back (charity, not profit: "it's not for us, all the money goes to charity"). A consistent BvK community-first value.

*Bron: blogpost 'Our Byte-Sized book is AVAILABLE for download - FREE of charge' (2019-10-14), basvankaam.com.*

## Verwante notities

- [Byte-Sized: Cloud Design Principles and Architectural Recommendations book](byte-sized-cloud-design-book.md)
- [Project Byte-Sized: a crowd-authored cloud book for charity](byte-sized-community-book.md)
- [Stance: curating the community's best content is a contribution worth making](community-curation-as-contribution.md)
- [Stance: take a vendor job but keep it community-first](community-first-vendor-role.md)
- [An idea born small can grow huge fast](community-small-ideas-grow-big.md)
- [Community involvement as the engine of his career](positie-community-as-career-engine.md)
- [The community's success depends on us; give before you take](stance-give-to-the-community.md)
- [Treating the job as a hobby you'd do anyway](work-as-hobby-philosophy.md)
