---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [paper, cloud-first, hybrid, thesis, opinion]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# Bas's paper: Cloud Only? No. Cloud First or Hybrid? Yes.

Chapter 17 reproduces a paper Bas wrote for a Cloud Masterclass at the end of 2020, graded an 8 (he says "9+, well, an 8"). The lower mark was deliberate: he knowingly left out some mandatory sections, confident he'd still pass comfortably, and it saved him a full day of work — "Work smarter, not harder."

Bas adds an honest meta-reflection: rereading it years later he no longer fully agrees with some of his own statements — things change fast and that's fine; you're allowed to change your mind and move with the times ("if you do what you always did, you get what you always got, and that's no progress"). He also finds the paper somewhat negatively written and rather direct/blunt — clear, but not a communication style everyone handles well — something he's worked on in the years since.

The paper's core thesis: the cloud is not a silver bullet. A Cloud-only world is a Utopia, at least for the coming 15-20 years, possibly longer. The only valid path forward is **Cloud first or hybrid** — use cloud services where it makes sense and adds something. Step by step is the only correct approach. The paper covers: cloud characteristics (the promise), the human factor, traditional (Windows) applications, workspace aggregators, physical and software limitations, and the lack of overview/knowledge/governance.

## Bron-citaten (NL, verbatim)

> Cloud only? Nee. Cloud first of hybride? Ja. De enige weg voorwaarts.
>
> ‘Work smarter, not harder’
>
> De Cloud is geen silver bullet – de oplossing tot alles.
>
> een Cloud only wereld een Utopia is

## Verwante notities

- [Bas on the Cloud roadmap as a strategic tool](bas-cloud-roadmap-as-strategic-tool.md)
- [Bas deconstructs the cloud promises](bas-deconstructs-the-cloud-promises.md)
- [Bas's list of reasons companies stay (partly) on-premises](bas-objections-to-full-cloud-migration.md)
- [The human factor: loss of control and trust](bas-on-the-human-factor-and-loss-of-control.md)
- [Correction: The Cloud-First Paper Dates to 2019, Not 2020](bas-paper-actually-written-2019-ncoi-masterclass.md)
- [Paper conclusion: Cloud only? Maybe never](bas-paper-conclusion-cloud-only-never.md)
- [Bas on the future limits of self-hosting your own data growth](bas-self-hosting-data-growth-limit.md)
- [Bas's reframe: call them 'traditional' applications, not 'Legacy'](bas-traditional-not-legacy-applications.md)
- [The 'Cloud because of the Cloud' syndrome and poor migration planning](cloud-because-of-cloud-syndrome.md)
- [Cloud deployment models (public, private, hybrid, community)](cloud-deployment-models.md)
- [Cloud is a potential solution, not a silver bullet](cloud-is-no-silver-bullet.md)
- [Cloud's origin around 2006 (Bas's paper, written 2020)](cloud-origin-around-2006-paper.md)
- [Full Cloud Migration Is Rarely a Good Idea](full-cloud-is-rarely-a-good-idea.md)
- [Hybrid cloud: best of both worlds via ExpressRoute and low latency](hybrid-cloud-best-of-both-worlds.md)
- [Bas on cloud, hybrid and the future of on-premises VDI](is-cloud-vdi-still-relevant-takeaways.md)
- [Lack of overview, knowledge, governance and Shadow IT](lack-of-overview-knowledge-governance-shadow-it.md)
- [Bas's mini business case: the current-vs-possible mapping table](mini-business-case-current-vs-hybrid-table.md)
- [Opdracht 2 - Tailored Cloud strategy: the puzzle of models and goals](opdracht-2-cloud-strategy-models-and-goals.md)
- [Cloud-only is a utopia; cloud-first or hybrid is the only honest answer](positie-cloud-only-is-a-utopia.md)
- [The promises of the Cloud (and Bas's invitation to challenge them)](the-promises-of-the-cloud.md)
- [Why traditional Windows applications won't disappear](traditional-windows-applications-wont-disappear.md)
- [When the Cloud is a good solution - and when it is not](when-cloud-is-and-isnt-a-good-fit.md)
