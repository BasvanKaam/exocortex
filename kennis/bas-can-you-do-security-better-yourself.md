---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [positie, security, framing, cloud]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# Bas's challenge: can you do cloud security better yourself?

After laying out that hyperscalers invest billions annually (e.g. Microsoft's announced 20 billion USD over five years) into cloud security, Bas turns it into a pointed reframe aimed at the reader who considers keeping everything in-house. He asks you to wonder whether you can really do this better yourself: 'Wat is jullie jaarlijkse budget voor (Cloud) veiligheid en hoeveel uur per week of maand besteed je hier aan?' — What is your annual budget for (cloud) security, and how many hours per week or month do you spend on it? The implicit argument: the hyperscalers' security spend is part of the value you get and is hard to match independently.

## Bron-citaten (NL, verbatim)

> Je kunt je afvragen of je dit zelf beter kunt. Wat is jullie jaarlijkse budget voor (Cloud) veiligheid en hoeveel uur per week of maand besteed je hier aan?

## Verwante notities

- [Bas deconstructs the cloud promises](bas-deconstructs-the-cloud-promises.md)
- [Bas's list of reasons companies stay (partly) on-premises](bas-objections-to-full-cloud-migration.md)
- [The human factor: loss of control and trust](bas-on-the-human-factor-and-loss-of-control.md)
- [Key Takeaways Chapter 10 (Cloud Security)](chapter-10-security-key-takeaways.md)
- [Hyperscaler security investments and certifications](cloud-provider-security-investments.md)
- [Gartner: 99% of security issues are caused by people](gartner-99-percent-security-issues-are-people.md)
- [Performance and Stability: You Trade Control for Provider Scale](performance-stability-and-provider-scale.md)
- [Bigger cloud providers are not more secure by default](position-bigger-cloud-providers-not-more-secure.md)
- [Shared Responsibility Model (Restaurant Analogy)](shared-responsibility-model-restaurant-analogy.md)
- [The promises of the Cloud (and Bas's invitation to challenge them)](the-promises-of-the-cloud.md)
