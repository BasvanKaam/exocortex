---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [citrix, printing, opinion, euc]
bron: basvankaam-blog
---

# Printing is business critical, and underrated

Bas openly likes printing, an unfashionable EUC opinion he owns (June 2016): "First off, I like printing." Not because he's built many complex print architectures (a couple, far from many) but because he likes the technology and the processes behind it. His core argument: printing is considered business critical at almost every customer he's visited, even if people underrate it. He illustrates with everyday behaviour, users print the important email or whitepaper, annotate it, take it home. He admits he's biased: "Although I prefer pen and paper over a keyboard as well, so perhaps I'm not the best reference." The stance matters because it frames why he treats printing improvements (like 7.9 UPS HA) as significant rather than minor, and shows his willingness to champion the boring-but-essential parts of the stack.

*Bron: blogpost 'What's new... Printing in Citrix XenDesktop and XenApp 7.9? A more in-depth look.' (2016-06-03), basvankaam.com.*

## Verwante notities

- [Bas on why he likes Citrix printing](bas-likes-printing.md)
- [Bas's research workflow: pen-and-paper first, then digitalise](bas-pen-to-digital-research-workflow.md)
- [Take notes by hand: the pen beats the keyboard](handwriting-beats-keyboard.md)
- [Stance: no public PDF of the book, full stop](no-pdf-no-piracy-stance.md)
- [Most Citrix printing pain is still bad drivers and bad architecture](positie-citrix-printing-still-driver-and-design-problem.md)
- [Print architecture is decided by all elements combined, not one knob](print-architecture-is-all-elements-combined.md)
- [Printing is the perennial pain admins shouldn't have to focus on](printing-is-a-perennial-euc-pain.md)
- [Printing is still critical; Citrix deprioritized it for cloud](printing-still-critical-citrix-deprioritized-it.md)
- [Universal Print Server load balancing and HA (7.9)](ups-load-balancing-79.md)
