---
type: positie
merk: bvk
domein: nerdio
status: actief
datum: 2026-06-11
tags: [positie, events, enablement, community, hands-on-labs]
bron: nerdio-content
---

# Lead enablement with hands-on labs, not slideware

Across every event format Bas designs, the hands-on lab is the point and the presentation is the setup. Openings and slide segments are deliberately short (10-45 minutes); the bulk of the time is people doing the work in a real environment. The more community-focused the audience, the more this holds: the Tech Insider format carries an explicit NO marketing rule and a 10-15 minute intro cap, because the moment a community session feels like a pitch, you lose the room. Marketing intensity should scale inversely with how community-driven the audience is.

*Bron: Nerdio-content 'SE Huddle' (Innovation Days).*

## Verwante notities

- [Two event types: Training Camp vs Innovation Day](etc-vs-innovation-day-event-design.md)
- [Idea: Your Nerdio configuration as your best sales tool](idee-nerdio-config-as-sales-tool.md)
- [Method: The Learning Pyramid Applied to Technical Training](learning-pyramid-training-method.md)
- [Method: Bas's Nerdio lab-guide template convention](method-nerdio-lab-guide-template.md)
- [The Innovation Day / MicroHack hands-on event format](nerdio-innovation-day-microhack-format.md)
- [Show, don't sell](positie-show-dont-sell.md)
- [Active Learning Beats Passive: Design Training for Retention, Not Coverage](position-active-learning-beats-passive.md)
- [XpressLabs facilitation craft: how Bas runs the room](xpresslabs-facilitation-craft.md)
- [Zero Trust session method: lead with the weakest link](zero-trust-session-method-weakest-link.md)
