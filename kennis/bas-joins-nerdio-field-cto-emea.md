---
type: kennis
merk: bvk
domein: nerdio
status: actief
datum: 2026-06-10
tags: [career, nerdio, field-cto, timeline]
layer: reference
gedateerd: nee
bron: basvankaam-blog
---

# Bas joined Nerdio as Field CTO EMEA on 1 June 2020

Key biographical anchor. As of 01-06-2020 Bas van Kaam joined Nerdio in the role of Field CTO for the EMEA region, leaving Salomon IT. At the time he was an active Microsoft MVP in the Enterprise Mobility category (which then included RDS and WVD). His mandate as described: talk to customers/partners about building a Cloud practice on Azure, feed field feedback back to the Nerdio team, build out the community brand, on-board channel partners, and run webinars/articles/presentations/events. This is the start of the Nerdio chapter of his career.

*Bron: blogpost 'Level Up - Joining Nerdio as Field CTO EMEA' (2020-06-02), basvankaam.com.*

## Verwante notities

- [Bas van Kaam 2020 career milestones](bas-2020-career-milestones.md)
- [Bas leans into hyper-convergence and web-scale as the future](bas-on-web-scale-as-future-of-it.md)
- [Bet: WVD third-party vendors are where the action is](betting-on-wvd-third-party-tooling-2020.md)
- [Stance: take a vendor job but keep it community-first](community-first-vendor-role.md)
- [Framing: the product's job is hiding complexity](hiding-complexity-is-the-product.md)
- [Liquidware is a US success but underrated in Europe](liquidware-underrated-in-europe.md)
- [Nerdio Manager for WVD roadmap as of mid-2020](nerdio-2020-product-roadmap-snapshot.md)
- [Nerdio Manager for WVD launched March 2020 with fast traction](nerdio-manager-wvd-early-traction.md)
- [Nerdio innovating on top of WVD is 'a perfect marriage'](nerdio-plus-microsoft-perfect-marriage.md)
- [Microsoft's WVD power scripts are not real autoscaling](nerdio-vs-microsoft-power-management-scripts.md)
- [Thesis: Nerdio on top of WVD is a 'perfect marriage'](nerdio-wvd-perfect-marriage-thesis.md)
- [Treating the job as a hobby you'd do anyway](work-as-hobby-philosophy.md)
