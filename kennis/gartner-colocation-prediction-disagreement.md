---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-09
tags: [colocation, gartner, positie, prediction]
layer: rich
bron: van-de-basis-tot-meester-in-de-cloud
---

# Bas Disagrees With Gartner on Co-location's Demise

A pointed Bas opinion: Gartner has said for several years now that the co-location model will disappear from the market fairly quickly. Bas's reaction — "ik zie dat niet snel gebeuren" (I don't see that happening any time soon). A small example of his willingness to push back on analyst consensus from his own field experience.

## Bron-citaten (NL, verbatim)

> Overigens zegt Gartner al een aantal jaar dat dit model vrij snel van de markt zal verdwijnen, ik zie dat niet snel gebeuren.

## Verwante notities

- [Bas's COVID-era asides and skepticism on the stats](bas-covid-asides-and-skepticism.md)
- [Bas's list of reasons companies stay (partly) on-premises](bas-objections-to-full-cloud-migration.md)
- [Paper conclusion: Cloud only? Maybe never](bas-paper-conclusion-cloud-only-never.md)
- [Bas on the future limits of self-hosting your own data growth](bas-self-hosting-data-growth-limit.md)
- [The Gartner Hype Cycle and its five stages](gartner-hype-cycle.md)
- [Four Hosting Models - On-Prem, Co-location, Managed, Cloud](hosting-models-onprem-colo-managed-cloud.md)
- [Tom Siebel's scepticism that the Cloud survived](tom-siebel-cloud-scepticism.md)
