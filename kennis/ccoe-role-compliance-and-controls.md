---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [ccoe, compliance, controls, shared-responsibility, raci]
layer: reference
gedateerd: nee
attributie: weolcan-wcgf
bron: salomon-cloud-governance
---

# CCoE role and responsibility demarcation for compliance

> Referentie uit het Weolcan Cloud Governance Framework (WCGF). Niet Bas zijn eigen IP, hier bewaard als naslag.

For compliance and risk, the Cloud Center of Expertise (CCoE) supports teams by helping with design, exchanging knowledge, informing them of relevant product updates, and helping set up the proper controls.

Responsibility demarcation:

- Determine who is responsible for security in the cloud.
- The CCoE assists with selecting and setting up proper cloud controls.

Governance to-do list for this domain: create security and risk response scenarios for cloud service chains, and draw up KPIs to monitor automation success and failure.

*Bron: Salomon-IT 'Compliance and Risk - Cloud Governance'.*

## Verwante notities

- [CCoE role in event monitoring and incident management](ccoe-event-monitoring-role.md)
- [The CCoE at the heart of the Target Operating Model](ccoe-in-target-operating-model.md)
- [The CCoE's role in AVG compliance and governance](ccoe-role-in-avg-compliance.md)
- [CCoE Roles and Responsibilities](ccoe-roles-and-responsibilities.md)
- [Compliance and risk as operational governance](compliance-risk-operational-governance.md)
- [Blind faith in automation will not ensure happy outcomes](position-stay-in-control-of-your-control.md)
- [Risk Analysis: Control Measures and Decision-Making](risk-analysis-control-measures.md)
