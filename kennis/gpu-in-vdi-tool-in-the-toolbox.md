---
type: kennis
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [gpu, vdi, ux, design]
layer: reference
gedateerd: nee
bron: basvankaam-blog
---

# GPUs in VDI: a tool in the toolbox, not a default

GPU adoption in VDI/SBC was slow because GPUs were (too) expensive and a properly designed environment performs fine without one. A GPU boosts overall UX but adds another layer to set up and manage. Treat it as 'a tool in the toolbox': use it where it makes sense, not in every design. Durable design principle for sizing virtual desktops.

*Bron: blogpost 'A few 'State of the VDI and SBC union' results in more detail' (2017-07-23), basvankaam.com.*

## Verwante notities

- [Use-cases for VDI/desktop benchmarking](benchmarking-use-cases.md)
- [Blast Extreme: ports, TCP vs UDP, and GPU offload numbers](blast-extreme-network-and-gpu-behaviour.md)
- [Intel entering vGPU turns the GPU space into a three-way race](intel-enters-vgpu-three-way-race.md)
- [Plenty of room left for innovation in HCI, GPU and app delivery (2015)](room-for-innovation-hci-gpu-app-delivery.md)
- [Categorize users before choosing a desktop model](user-categorization-for-desktop-design.md)
