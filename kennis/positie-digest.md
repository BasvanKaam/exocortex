---
type: index
merk: bvk
domein: persoonlijk
status: actief
datum: 2026-06-19
tags: [index, posities, schrijven, voice, navigatie]
---

# Positie-digest: signature stances (schrijf-kompas)

De schrijf-brandstof. Dit is geen volledige lijst van alle 484 posities (die staan per domein in de domein-indexen, sectie Posities), maar een curated kaart van de cross-cutting, overdraagbare stances waar Bas zijn content uit schrijft: LinkedIn, blog, talks, podcast. Pak bij een onderwerp het bijbehorende cluster, zodat een post op een samenhangend, doorleefd standpunt staat in plaats van losse feiten.

Gebruik: kies een thema, lees de stances, pak er een of twee die de kern raken. Combineer met de voice-laag (`standaarden/voice/`) voor de toon.

## Cloudstrategie en de cloud-mythes
- [There is no such thing as a cloud strategy](cloud-is-not-a-strategy.md)
- [Cloud only? No. Cloud first or hybrid? Yes](position-cloud-first-not-cloud-only.md)
- [Hybrid Is the Dominant Model, So Build the Best Bridge](hybrid-is-the-dominant-model-build-the-bridge.md)
- [Full Cloud Migration Is Rarely a Good Idea](full-cloud-is-rarely-a-good-idea.md)
- [The Cloud Is Not Automatically Cheaper](cloud-is-not-automatically-cheaper.md)
- [Cloud success is an organisational and adoption problem, not a technical one](cloud-success-is-organisational-not-technical.md)
- [Cloud is a potential solution, not a silver bullet](cloud-is-no-silver-bullet.md)
- [Plan the Exit Before You Commit](plan-the-exit-before-you-commit.md)
- [Compare cloud to cloud, not cloud to on-prem](cloud-fit-compare-cloud-to-cloud.md)
- [Cloud is not a different way of working, it demands a different way of thinking](cloud-demands-different-thinking-not-different-working.md)

## Vendor-onafhankelijkheid en anti-hype
- [We Give the Insight, You Make the Choice](give-insight-client-decides.md)
- [We sell a vision, not hardware or software](position-we-sell-a-vision-not-hardware-or-software.md)
- [Don't let any single vendor spook you](positie-dont-let-any-single-vendor-spook-you.md)
- [At the Bigger Picture, Stay Vendor-Agnostic](positie-vendor-agnostic-at-the-bigger-picture.md)
- [Don't think in brands or product boxes](avoid-vendor-and-product-thinking.md)
- [Don't be fooled by DaaS marketing](dont-be-fooled-by-daas-marketing-position.md)
- [Summarize the features, spare people the marketing propaganda](skip-the-marketing-propaganda.md)
- [Choice and competition are good in the EUC market](choice-and-competition-are-good-for-euc.md)

## Business vóór technologie
- [Technology Should Never Be Leading](technology-should-never-be-leading.md)
- [Put the Business Before the Technology](business-before-technology.md)
- [Functionality before technology; business objectives drive cloud adoption](functionality-before-technology-ea-principle.md)
- [Convergence, Not Alignment, Between Business and IT](business-it-convergence-not-alignment.md)
- [Cloud adoption goes far beyond technology alone](position-cloud-adoption-is-more-than-tech.md)
- [Technology Still Matters, and Is Getting More Complex, Not Less](technology-still-matters-and-is-getting-more-complex.md)
- [It should be Business-IT (BIT), not IT alone](position-business-technology-over-it.md)

## Kosten, waarde en FinOps
- [If cost saving is your primary cloud driver, reconsider your strategy](cost-saving-is-the-wrong-primary-cloud-driver.md)
- [Is the Cloud Cheaper? It Depends](is-cloud-cheaper-it-depends.md)
- [Do Not Fixate on the Financials Alone](dont-fixate-on-financials-alone.md)
- [The True ROI Is Time and Wellbeing, Not Just Money](position-true-nerdio-roi-is-time-and-wellbeing.md)
- [Lead with operational and business value; cost savings is the cherry on top](lead-with-operational-value-not-just-cost.md)
- [Fast, Good, Cheap: You Can Now Pick All Three](fast-good-cheap-pick-three.md)
- [A cost-aware organization is the ultimate goal of cloud governance](position-cost-aware-organization-is-the-goal.md)
- [Measure to know (meten = weten)](measure-to-know-meten-is-weten.md)

## Remote en hybride werk, de werkplek
- [Work is a place, just not always the same place](work-is-a-place-just-not-always-the-same-place.md)
- [Remote work is fine, but nothing beats being there in person](remote-work-fine-but-in-person-beats-it.md)
- [Remote Work Is a Talent Requirement and an Opportunity](remote-work-as-opportunity-and-talent-magnet.md)
- [Make the Best of the Crisis: Prepare for Permanent Remote](make-the-best-of-the-crisis-prepare-for-permanent-remote.md)
- [WFH is not going away anytime soon](wfh-is-here-to-stay-2020.md)
- [An Office rollout is part of a wider workplace transformation](office-rollout-is-bigger-than-installing-software.md)
- [Home-office air quality and humidity are an underrated lever](home-office-air-quality-humidity-matter.md)
- [A coming recession will stall remote-work investment](recession-will-stall-remote-work-investment.md)

## Community, kennisdeling en leren
- [The community's success depends on us; give before you take](stance-give-to-the-community.md)
- [Freely sharing knowledge is the best advertising a company can get](free-knowledge-sharing-is-the-best-advertising.md)
- [Quality Over Quantity in Community Content](quality-over-quantity-in-community-content.md)
- [If you want to go far, go together](go-together-community-force.md)
- [Take a vendor job but keep it community-first](community-first-vendor-role.md)
- [Community involvement as the engine of his career](positie-community-as-career-engine.md)
- [Be a Learn-It-All Organization, Not a Know-It-All One](learn-it-all-not-know-it-all.md)
- [Community sessions should be technical substance, not marketing](events-substance-over-marketing.md)

## AI en content-productie
- [Using AI voices to deliver his content](ai-voices-for-content-delivery.md)
- [Good Is Good Enough: Ship Content, Don't Polish to Perfection](good-is-good-enough-for-content.md)
- [Bas's GitHub and the AI tooling he builds](bas-github-ai-tooling.md)
- [Second brain als cognitieve infrastructuur](second-brain-cognitive-infrastructure.md)

## Hoe hij adviseert (consulting-methode)
- [Start small, test, then scale, because it de-risks the client's investment](position-start-small-test-then-scale-derisks-investment.md)
- [The Two-Scenario Offer: Centralized Backend vs Managed-Laptop-Only](two-scenario-offer-pattern.md)
- [Aftercare and knowledge transfer are the real differentiator](stance-aftercare-is-the-differentiator.md)
- [Against Jargon-Heavy, 100-Page Deliverables](against-jargon-heavy-deliverables.md)
- [Design for Restore, Not Backup (RPO/RTO/MTPD First)](design-for-restore-not-backup.md)
- [Interview questions are handholds, not a checklist](interviewvragen-zijn-handvatten-geen-checklist.md)
- [Begin at the beginning](start-from-where-you-are.md)
- [Get informed before you commit, not after](informed-before-committing-to-cloud.md)

## Security en risico
- [Security is a process, not a destination](security-is-a-process-not-a-destination.md)
- [Front door locked, back door wide open](front-door-locked-back-door-open.md)
- [Bigger cloud providers are not more secure by default](position-bigger-cloud-providers-not-more-secure.md)
- [Harden step by step and verify between steps](harden-step-by-step-not-all-at-once.md)
- [Nerdio Is Not Secure Out of the Box, So Always Harden](positie-nerdio-not-secure-out-of-the-box.md)
- [DR Is About Apps and Data, Not Fancy Infrastructure](positie-dr-is-about-apps-and-data.md)
- [Concentrated cloud knowledge in a few seniors is a business risk](knowledge-leaving-the-company-as-a-real-risk.md)

## Nerdio en AVD-overtuigingen
- [Microsoft has won the VDI war; AVD is the DaaS leader](microsoft-has-won-the-vdi-war.md)
- [Nerdio and Microsoft: a perfect marriage, not a competitor](nerdio-microsoft-perfect-marriage.md)
- [The product's job is hiding complexity](hiding-complexity-is-the-product.md)
- [The real win of WVD automation is reclaimed admin time, not just compute](automation-saves-time-equals-money-position.md)
- [Auto-scale first, then reserve the residual](auto-scale-before-reserved-instances.md)
- [Good UI is a business strategy, not decoration](positie-ui-as-product-strategy.md)
- [Standing up WVD is the easy 10%; the hard part is everything after](wvd-setup-is-easy-operations-is-hard.md)
- [Why customers choose Nerdio: the pillars](why-customers-choose-nerdio-pillars.md)
- [MSPs should charge per outcome, not per hour](positie-charge-per-outcome-not-per-hour.md)
- [Show, don't sell](positie-show-dont-sell.md)

## Carrière en persoonlijke filosofie
- [Persistence beats everything: career and life advice](persistence-over-everything-career-advice.md)
- [Don't make money your number one priority](money-not-number-one.md)
- [Celebrate your failures, not only your successes](celebrate-failures-not-just-successes.md)
- [Be yourself; impersonating someone else never lasts](be-yourself-authenticity.md)
- [Treating the job as a hobby you'd do anyway](work-as-hobby-philosophy.md)
- [Work-life isn't a balance; work is one part of life's whole package](work-is-part-of-life-not-balanced-against-it.md)
- [Schedule deliberate Think Weeks to step back and think](think-week-deliberate-time-to-think.md)
- [Public speaking: terrified every time, but totally worth it](positie-public-speaking-worth-the-nerves.md)
- [The bigger the problem, the slower you respond](position-slow-down-on-big-problems.md)

## Schrijven, self-publishing en content-strategie
- [It's my book, it needs to sound like me](it-needs-to-sound-like-me.md)
- [Self-publish first, for the freedom](why-self-publish-a-tech-book.md)
- [Publish the draft and let the community fact-check it](publish-the-draft-crowdsource-the-facts.md)
- [Publish only when there is something to say, not to stay visible](publish-only-with-added-value-not-for-visibility.md)
- [More Is Not By Definition Better](more-is-not-by-definition-better.md)
- [Just start writing, the topic and quality come by doing](just-start-writing-it-comes-by-doing.md)
- [The "I'm no guru" philosophy: blogging as a learning tool, not authority](no-guru-philosophy.md)
- [Bulleted lists are one of the best ways to share knowledge](lists-are-the-best-way-to-share-knowledge.md)

## EUC en technische signature-stances
Het grootste blok (211 EUC-posities) is overwegend technisch en staat volledig in [euc - index](index-euc.md). De handvol die telkens terugkomt in zijn verhaal:
- [There is no single right way to deliver apps and desktops](no-single-right-way-to-deliver-apps-desktops.md)
- [RDP Suffices in 70%+ of Cases, So Look Beyond Citrix/VMware](position-rdp-suffices-look-beyond-citrix-vmware.md)
- [Printing is business critical, and underrated](printing-is-business-critical.md)
- [You don't need Citrix Support all the time, learn to troubleshoot the FMA yourself](dont-need-citrix-support-all-the-time.md)
- [In a hybrid world, app virtualization beats desktop virtualization](app-virtualization-over-desktop-virtualization-in-hybrid.md)
- [Windows 10 was never the promised 'One OS to rule them all'](windows-10-not-one-os-to-rule-them-all.md)
- [Call them traditional Windows apps, not legacy](positie-windows-apps-traditional-not-legacy.md)
- [The 80/20 'two kinds of sysadmin' framework](two-kinds-of-sysadmin-80-20.md)
- [Listen to the employee, don't let IT dictate how they work](listen-to-the-employee-not-it-dictates.md)

## Volledige lookup
Voor alle 484 posities, per domein: [euc](index-euc.md), [cloud](index-cloud.md), [nerdio](index-nerdio.md), [financieel](index-financieel.md), [persoonlijk](index-persoonlijk.md) (elk met een sectie Posities).

## Verwante notities
- [Brein-index (master)](index.md)
- [persoonlijk - index](index-persoonlijk.md)
- [Writing-pack generator (recept)](../standaarden/workflows/writing-pack-generator.md)
