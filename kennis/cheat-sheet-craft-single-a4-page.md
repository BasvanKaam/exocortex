---
type: positie
merk: bvk
domein: euc
status: actief
datum: 2026-06-10
tags: [application-layering, cheat-sheet, craft, stance]
bron: basvankaam-blog
---

# Stance: keep the comparison cheat sheet to a single A4 page

In this Feb 2017 release of v3.0, Bas revealed a deliberate design constraint that shaped his recurring cheat sheets: keep it to a single A4 page for as long as possible. To make v3.0 fit he deleted some older data for the first time and shrank the font from 7.5 to 7 to free up space. He was also explicit about the limits of the format: 'this is by no means meant as an exclusive list' and he could 'never be able to include all changes, features, and so on of every product.' It shows his maker mindset, ongoing maintenance commitment, and honesty about a reference's boundaries.

*Bron: blogpost 'The application layering technology cheat sheet, version 3.0!' (2017-02-28), basvankaam.com.*

## Verwante notities

- [Application layering narrows to the big three (2017)](app-layering-big-three-2017.md)
- [Application layering vendor landscape circa 2016](application-layering-vendor-landscape-2016.md)
- [AVD Security Guidelines cheat sheet: a 35-item starting point](avd-security-cheat-sheet-starting-point.md)
- [A cheat sheet should orient, not rank vendors](cheat-sheet-as-starting-point-not-comparison.md)
- [The cheat-sheet method: one A4 to compare a crowded field](cheat-sheet-comparison-method.md)
- [How the conferencing cheat sheet was researched](conferencing-cheat-sheet-method.md)
- [FSLogix is not an application layering company; App Disks are doomed](fslogix-not-an-app-layering-company.md)
- [IMA-to-FMA terminology: Farm to Site, Data Collector to Delivery Controller](ima-to-fma-terminology-map.md)
- [Bulleted lists are one of the best ways to share knowledge](lists-are-the-best-way-to-share-knowledge.md)
- [Publish the draft and let the community fact-check it](publish-the-draft-crowdsource-the-facts.md)
- [Remote video-conferencing cheat sheet (v1.0, 2020)](remote-conferencing-cheat-sheet-2020.md)
- [Frame security guidance as a living starting point, not a final word](security-checklist-as-living-starting-point.md)
