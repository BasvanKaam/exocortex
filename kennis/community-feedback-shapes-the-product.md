---
type: positie
merk: bvk
domein: nerdio
status: actief
datum: 2026-06-11
tags: [community, feedback, agile, product, positie]
bron: nerdio-blog
---

# Community feedback should visibly shape the product

Bas's stance: a vendor community is only worth running if members can see their input shaping the product. He argues for taking feedback seriously enough that it shows up in the software, backed by an agile cadence (monthly releases, an active roadmap) so the loop is real, not performative. He explicitly credits the community as a reason the products are shaped the way they are. The corollary is quality over quantity: a small, engaged group beats a large passive one, and the program should never become a sales channel in disguise.

*Bron: Nerdio-content 'Introducing - The Nerdio Tech Insider Program' (Blogs).*

## Verwante notities

- [Bas's contrarian tip: get feedback from prospects you lost, not just customers](bas-learn-from-lost-prospects.md)
- [Consistency Beats Volume in Community Advocacy](consistency-beats-volume-in-community-advocacy.md)
- [Structured Idea / Feature-Request Form as a Product Feedback Loop](idea-form-feedback-loop.md)
- [Idee: Nerdio Tech Insider Community Program](idee-nerdio-tech-insider-community-program.md)
- [Be a Learn-It-All Organization, Not a Know-It-All One](learn-it-all-not-know-it-all.md)
- [Community feedback should shape the product](positie-community-shapes-the-product.md)
- [Don't hide costs from customers: full-disclosure pricing builds trust](radical-pricing-transparency-position.md)
- [Co-build the Citrix-to-Nerdio course, then turn members into authors](community-co-built-course-and-author-ownership.md)
