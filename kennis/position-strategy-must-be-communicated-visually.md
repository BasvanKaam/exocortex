---
type: kennis
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [cloud-governance, position, strategy-map, communication]
layer: reference
gedateerd: nee
attributie: weolcan-wcgf
bron: salomon-cloud-governance
---

# Position: A Strategy Nobody Knows Is Worthless

> Referentie uit het Weolcan Cloud Governance Framework (WCGF). Niet Bas zijn eigen IP, hier bewaard als naslag.

the framework leans on the finding that 95% of employees are unaware of their organization's strategic objectives to argue that strategy communication is a first-class problem, not a soft extra. His position: an IT strategy only contributes to organizational goals if it is actually understood across the organization.

He prescribes a concrete instrument, the Kaplan & Norton Strategy Map, as both the modeling method and the communication artifact, and insists the result must be readable without extensive explanation. Strategy that lives only in leadership's heads does not steer day-to-day activity.

*Bron: Salomon-IT 'ICT Vision - Cloud Governance'.*

## Verwante notities

- [Governance Outcomes by Level and the Role of Communication](governance-outcomes-by-level-and-communication.md)
- [Position: No Information, No Control, No Trust](position-control-information-trust-chain.md)
- [Strategy Map (Kaplan-Norton) within SCAF Strategy phase](scaf-strategy-map-kaplan-norton.md)
- [SCAF support tool: the strategy map (read up, build down)](scaf-strategy-map-support-tool.md)
- [SCAF strategy map: a balanced-scorecard layering for the strategy phase](scaf-strategy-map-teaching-artifact.md)
- [SCAF visual timeline and Bas's preference for visual artifacts](scaf-visual-timeline.md)
- [Strategy Map (Kaplan & Norton) for IT Strategy Communication](strategy-map-kaplan-norton-for-it-strategy.md)
