---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-10
tags: [avd, security, community, documentation]
bron: basvankaam-blog
---

# Frame security guidance as a living starting point, not a final word

When releasing his AVD security cheat sheet (April 2023), Bas deliberately positioned it as a starting point and a work in progress rather than an authoritative complete reference. He explicitly states it's by no means his intention to provide a list of all security features available, and he openly invites the community to tell him what to add, change, or remove so it can keep evolving. This reflects a recurring stance: ship a useful v1.0, credit others' work (here, Aaron Dinnage's M365 license overview), and let the community iterate rather than waiting for completeness.

*Bron: blogpost 'The Azure Virtual Desktop Security Guidelines Cheat Sheet - v1.0' (2023-04-11), basvankaam.com.*

## Verwante notities

- [AVD security cheat sheet: the six-category framework](avd-security-cheat-sheet-framework.md)
- [AVD Security Guidelines cheat sheet: a 35-item starting point](avd-security-cheat-sheet-starting-point.md)
- [Security is always a fine line against user experience](avd-security-vs-user-experience-balance.md)
- [A cheat sheet should orient, not rank vendors](cheat-sheet-as-starting-point-not-comparison.md)
- [Stance: keep the comparison cheat sheet to a single A4 page](cheat-sheet-craft-single-a4-page.md)
- [Community feedback as the force that shapes the product](community-as-product-shaping-force.md)
- [Publish the draft and let the community fact-check it](publish-the-draft-crowdsource-the-facts.md)
- [The community's success depends on us; give before you take](stance-give-to-the-community.md)
- [Wireless Encryption: WEP, WPA and WPA2](wireless-encryption-wep-wpa-wpa2.md)
