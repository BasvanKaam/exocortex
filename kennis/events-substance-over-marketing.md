---
type: positie
merk: bvk
domein: cloud
status: actief
datum: 2026-06-11
tags: [come-get-it, events, community, opinion, vendor-marketing]
bron: cgit-blog
---

# Stance: community sessions should be technical substance, not marketing

When curating Come Get IT, Bas deliberately picks speakers and sessions that go deep on the technology and leave the vendor marketing aside. He frames a NVIDIA session as worthwhile precisely because it covers the markets and tech "waarbij we de marketing praat even laten voor wat het is" (where we leave the marketing talk for what it is). His implicit standard for a good community session: real practitioners, real architecture and tips, bulletpoint-light, humour welcome, hype not.

*Bron: Salomon-IT 'Uitgelicht! Come Get IT event spreker Bart Groot Zevert' (CGIT Blogs).*

## Verwante notities

- [Come Get IT event: Bas's community evening format](come-get-it-event-format.md)
- [Good Is Good Enough: Ship Content, Don't Polish to Perfection](good-is-good-enough-for-content.md)
- [Stance: the best technical speakers bridge IT and the business and cut to the essence](ideal-speaker-bridge-it-and-business.md)
- [Stance: lead with the pitfalls instead of routing around them](lead-with-pitfalls-not-around-them.md)
- [Personal Invitations Beat Mass Sign-Up](personal-invitations-beat-mass-signup.md)
- [At the Bigger Picture, Stay Vendor-Agnostic](positie-vendor-agnostic-at-the-bigger-picture.md)
- [Quality Over Quantity in Community Content](quality-over-quantity-in-community-content.md)
- [Salomon's Two Event Formats: Internal Technical vs Bigger-Picture](salomon-event-series-two-event-types.md)
- [Stance: quality over quantity for community content](stance-cgit-quality-over-quantity.md)
- [Stance: conference attendance must be justified by shareable output](stance-events-justified-by-shareable-output.md)
- [Idea: build a flagship demo (F1 sim on VDI) to prove a technology's limits](vdi-f1-simulator-showcase-idea.md)
- [Vendor partner congresses are marketing-first roadmap events](vendor-congresses-are-marketing-roadmaps.md)
- [How a vendor behaves on social channels is a signal](vendor-tone-on-socials-matters.md)
