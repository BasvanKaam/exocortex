---
type: positie
merk: bvk
domein: nerdio
status: actief
datum: 2026-06-11
tags: [positie, user-experience, nerdio, product]
bron: nerdio-training
---

# Good UI is a business strategy, not decoration

Bas argues that interface quality is a commercial lever, not cosmetics. UX/UI is about building flow, reducing friction, and creating trust through every click; when you get it right it doesn't just feel better, it works better. Concretely, good UI deflects support calls, shortens onboarding, and accelerates sales. He frames the interface as an onboarding coach, a support deflector, a sales accelerator, and a platform for growth all at once, and notes that while he loves training, 'the interface often speaks for itself.'

*Bron: Nerdio-content 'Ideas' (Training).*

## Verwante notities

- [A lab is allowed to be boring (ease of use as a selling point)](a-lab-is-allowed-to-be-boring.md)
- [Critique: why isn't this in the GUI? (web.config friction)](build-it-into-the-gui-position.md)
- [Idea: Your Nerdio configuration as your best sales tool](idee-nerdio-config-as-sales-tool.md)
- [Idea: The psychology of Nerdio's interface design](idee-psychology-of-nerdio-ui-design.md)
- [The NMW thesis: automation and hiding complexity](nmw-thesis-automation-hiding-complexity.md)
- [Show, don't sell](positie-show-dont-sell.md)
- [Hide the Complexity, Broaden the Line Between UX and Cost](position-hide-complexity-broaden-the-line.md)
- [Simple doesn't mean easy (cloud ops and finances)](simple-doesnt-mean-easy.md)
- [Simplicity comes at the cost of flexibility](simplicity-costs-flexibility.md)
- [Support is a Differentiator, Not an Afterthought](support-as-a-differentiator-nerdio.md)
- [Teaching Philosophy: Simplify, Educate, Empower](teaching-philosophy-simplify-and-empower.md)
